#!/usr/bin/env python3
"""Compose the approved N=1024, K=fill twelve-path result and manifest."""
...
def main():
    ...
    for channel, lane in EXPECTED_PATHS:
        ...
        if len(source_rows) != 80:
            raise SystemExit(f"expected 80 rows, found {len(source_rows)}: {source}")
        ...
        if len(trace_rows) != 1920:
            raise SystemExit(f"expected 1920 trace rows, found {len(trace_rows)}: {trace}")
    ...
    manifest = {
        "decisions": ["CL-29", "CL-30", "CL-33", "CL-36", "CL-38", "CL-40"],
    ...
    with open(os.path.join(HERE, "results_manifest.json"), "w",
        ...
        handle.write("\n")
    print("composed 960 rows: 5 receivers, 16 SNR points, 12 panels")
