#!/usr/bin/env python3
"""Validate the approved N=1024, K=fill twelve-path SNR result."""
...
EXPERIMENT_ID = (
    "2026-08-05-red-snr-sweep-n1024-cp16-rate05-p5-10-dc10-kfill")
...
EXPECTED_GEOMETRY = ("1024", "16", "0.5", "5", "10", "10", "0")
...
EXPECTED_DECISIONS = {"CL-29", "CL-30", "CL-33", "CL-36", "CL-38", "CL-40"}
N512_HASHES = {
...
def require(condition, message):
    if not condition:
        raise SystemExit("N1024 fixed-configuration validation failed: " + message)
...
def main():
    ...
    for row in rows:
        ...
        require(int(row["payload_bits_per_frame"]) == 3656,
                "an aggregate row does not realize the approved K=fill payload")
        bits = int(row["payload_bits"])
        errors = int(row["bit_errors"])
        require(bits == int(row["payload_bits_per_frame"]) * 60,
    ...
    require(manifest["geometry_display"]["horizon"] == "0",
            "manifest does not display K=fill")
    ...
    require(page.count("K=fill") == 12, "not every panel displays K=fill")
    ...
    caption = "N=1024 · CP=16 · rate=0.5 · pilots=5/10 · dc=10 · K=fill"
    for channel, lane in EXPECTED_PATHS:
    ...
    view_builder = os.path.join(
        ...
        "2026-08-04-red-snr-sweep", "results", "build_sweep_view.py")
    namespace = {"__file__": view_builder, "__name__": "_n1024_view_builder"}
    ...
    n512_results = os.path.join(
        os.path.dirname(os.path.dirname(HERE)),
        "2026-08-04-red-snr-sweep-n512-cp16-rate05-p3-4-dc10-kfill",
        "results",
    )
    for name, expected in N512_HASHES.items():
        require(sha256(os.path.join(n512_results, name)) == expected,
                f"N512 result changed: {name}")

    n2048_results = os.path.join(
        os.path.dirname(os.path.dirname(HERE)),
        "2026-08-04-red-snr-sweep-n2048-cp128-rate05-p5-10-dc8-k4",
        "results",
    )
    for name, expected in N2048_HASHES.items():
        require(sha256(os.path.join(n2048_results, name)) == expected,
                f"N2048 result changed: {name}")

    print("N1024 fixed-configuration validation passed: 960 aggregate rows, "
          "23040 trace rows, 12 panels, 60 series")
