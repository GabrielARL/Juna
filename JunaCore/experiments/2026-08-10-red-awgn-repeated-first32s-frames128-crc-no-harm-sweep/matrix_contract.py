#!/usr/bin/env python3
"""Approved AWGN-023C geometry and result-shape contract."""

import re


FAMILY = "2026-08-10-red-awgn-repeated-first32s-frames128-crc-no-harm"
CAMPAIGN = "AWGN-023C"
EXPECTED_IDS = (
...
SNAPSHOT_INDICES = BASE_SNAPSHOT_INDICES * 4
AGGREGATE_ROWS_PER_PATH = 80
FRAME_TRACE_ROWS_PER_PATH = 10240
PROTECTED_TRACE_ROWS_PER_PATH = 4096


class MatrixContractError(RuntimeError):
    """The approved AWGN-023C matrix shape was not met."""
...
def parse_geometry(experiment_id):
    ...
    if match is None:
        raise MatrixContractError(
            "unexpected AWGN-023C experiment ID: " + experiment_id
        )
...
def validate_source_contract(payload):
    ...
    expected_measurement = {
        ...
        "base_snapshot_indices": list(BASE_SNAPSHOT_INDICES),
    ...
    if payload.get("payload_bits_per_point") != 206848:
        raise MatrixContractError("source contract payload-per-point differs")
...
if __name__ == "__main__":
    validate_ids()
    print(
        "AWGN-023C MATRIX CONTRACT PASS: 1 configuration, 12 paths, "
        "5 receivers, 0:2:30 dB, first 32 seconds, four passes, 128 frames, PFFT=4"
    )
