...
const DECISION = "AWGN-023C"
const FAMILY = "2026-08-10-red-awgn-repeated-first32s-frames128-crc-no-harm"
const CONFIGURATIONS = [(
    id="$FAMILY-n1024-cp64-rate025-p5-5-dc14-kfill-pfft4",
    config=(nfft=1024, cp=64, code_rate=0.25,
...
const EXPECTED_PAYLOAD_BITS_PER_FRAME = 1_616
const EXPECTED_PAYLOAD_BITS_PER_POINT = 128 * EXPECTED_PAYLOAD_BITS_PER_FRAME
const BASE_SNAPSHOT_INDICES = [
...
const SNAPSHOT_INDICES = repeat(BASE_SNAPSHOT_INDICES, 4)
const GENERIC_128_SNAPSHOT_INDICES =
    round.(Int, range(1, 2_973; length=128))

function _default_snapshot_positions(capture, packets::Integer,
    ...
    _, stop = B._capture_position_limit(capture, planning_length, rate)
...
# BenchmarkPort is freshly included under this experiment module, so this
# method replacement is private to the dedicated AWGN-023C process. The
# 128-frame branch repeats AWGN-022's positions. Other counts retain the
# existing distinct-position behavior, including AWGN-022's 32 positions.
@eval B begin
    function _snapshot_positions(capture::ReplayCapture, packets::Integer,
        ...
        if count == 128 && waveform_length == 9_536 && rate == 9_600.0 &&
                size(capture.h) == (768, 3_073) &&
                length(capture.phase) == 614_600 && capture.step == 200
            stop == 2_973 || throw(ArgumentError(
                "AWGN-023C capture position stop differs: $stop"))
            positions = $SNAPSHOT_INDICES
            all(position -> 1 <= position <= stop, positions) || throw(
                ArgumentError(
                    "AWGN-023C repeated packet position exceeds support"))
...
function _require_file_hash(path, expected, description)
    isfile(path) || error("AWGN-023C missing $description: $path")
    islink(path) && error("AWGN-023C refuses symlinked $description: $path")
    actual = file_sha256(path)
    actual == expected || error(
        "AWGN-023C $description content differs: expected $expected, got $actual")
end
...
function require_source_contract()
    ...
    package_root == realpath(joinpath(RECEIVER_REPOSITORY, "JunaCore")) ||
        error("AWGN-023C loaded JunaCore from $package_root")
...
function require_contract(; verify_data::Bool=true)
    length(CONFIGURATIONS) == 1 || error("AWGN-023C configuration count differs")
    length(PATHS) == 12 || error("AWGN-023C path count differs")
    collect(SNR_DB) == collect(0:2:30) || error("AWGN-023C SNR grid differs")
    FRAMES == 128 && CAPTURE_SECONDS == 32.0 && SEED == 4 || error(
        "AWGN-023C capture, frame, or seed contract differs")
    SNAPSHOT_INDICES == repeat(BASE_SNAPSHOT_INDICES, 4) || error(
        "AWGN-023C nested replay positions differ")
    length(BASE_SNAPSHOT_INDICES) == 32 &&
        allunique(BASE_SNAPSHOT_INDICES) &&
        !allunique(SNAPSHOT_INDICES) || error(
            "AWGN-023C repeated replay-pass contract differs")
    allunique(GENERIC_128_SNAPSHOT_INDICES) &&
        GENERIC_128_SNAPSHOT_INDICES != SNAPSHOT_INDICES || error(
            "AWGN-023C generic placement was not rejected")
    EXPECTED_PAYLOAD_BITS_PER_POINT == 206_848 || error(
        "AWGN-023C payload-per-point contract differs")
...
function _populate_capture_cache!(channel::Symbol, data_dir::AbstractString)
    ...
    isfile(file) || error("AWGN-023C missing measured capture: $file")
    islink(file) && error("AWGN-023C refuses symlinked measured capture: $file")
...
function require_snapshot_contract(capture, config)
    config == only(CONFIGURATIONS).config || error(
        "AWGN-023C snapshot contract received another configuration")
    size(capture.h) == (768, 3_073) || error(
        "AWGN-023C cropped tap geometry differs")
    length(capture.phase) == 614_600 || error(
        "AWGN-023C cropped phase geometry differs")
    frame_samples = 9_536
    _, stop = B._capture_position_limit(
        capture, frame_samples + F.PASSBAND_MODEM_PADDING_SAMPLES, A.MODEM_FS)
    stop == 2_973 || error("AWGN-023C capture position stop differs: $stop")
    ...
    maximum(ends) <= CAPTURE_SECONDS || error(
        "AWGN-023C replay support exceeds the first thirty-two seconds")
    (
...
function _configure_protected_receiver(id::Symbol, capture, config)
    constructor = id === :profiled_cz ?
        ...
        JunaCore.JunaCrcConditionedJointCwzFrame.Modulation :
        error("AWGN-023C unknown protected receiver: $id")
    ...
    Modulations.isvalid(receiver, capture.fc, A.MODEM_FS) || error(
        "AWGN-023C public receiver $id is invalid after configuration")
    receiver
end
...
function evaluate(capture, channel_id, lane, config, snr_db;
    ...
    B._noise_override() === nothing || error(
        "AWGN-023C refuses a non-AWGN noise override")
    ...
    function decode(item, payload_bits, decoder_input, fc, fs)
        ...
        if id in (:ofdm_fec, :pfft, :lite)
            id === :lite && item.receiver.cz_crc_no_harm && error(
                "AWGN-023C Lite changed at decode boundary")
    ...
    function benchmark_sink(row)
        ...
        if id in (:profiled_cz, :cwz_joint)
            haskey(trace_bundles, id) || error(
                "AWGN-023C protected decode produced no no-harm trace")
    ...
    isempty(trace_bundles) || error(
        "AWGN-023C did not consume every protected trace")
    [(
...
function validate_path_rows(rows, frame_traces, protected_traces;
    ...
    for snr in expected_snrs, frame in 1:FRAMES
        ...
        length(paired) == length(ALGORITHMS) || error(
            "AWGN-023C paired workload receiver count differs")
        length(Set((row.workload_id, row.snapshot_index, row.payload_seed,
            ...
            "AWGN-023C receivers do not share one paired workload")
    end
    for receiver in RECEIVER_IDS, snr in expected_snrs, base in 1:32
        passes = sort([
            row for row in frame_traces
            if row.algorithm_id == receiver && row.snr_db == snr &&
               mod1(row.frame, 32) == base
        ]; by=row -> row.frame)
        length(passes) == 4 || error(
            "AWGN-023C replay pass count differs")
        length(Set((row.snapshot_index, row.snapshot_seconds,
                    row.replay_support_end_seconds,
                    row.frame_duration_seconds, row.frame_samples)
                   for row in passes)) == 1 || error(
            "AWGN-023C replay window changed between passes")
        [row.payload_seed for row in passes] ==
            [SEED + base - 1 + 32pass for pass in 0:3] || error(
            "AWGN-023C pass payload seeds differ")
        [row.noise_seed for row in passes] ==
            [SEED + base - 1 + 32pass for pass in 0:3] || error(
            "AWGN-023C pass noise seeds differ")
        [row.replay_seed for row in passes] ==
            [SEED + base - 1 + 32pass for pass in 0:3] || error(
            "AWGN-023C pass replay seeds differ")
    end
    for receiver in RECEIVER_IDS, snr in expected_snrs
        ...
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-023C frame errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-023C frame successes do not reconcile")
    end
    for receiver in PROTECTED_RECEIVER_IDS, snr in expected_snrs
        ...
        frames = [row for row in protected_traces
                  if row.algorithm_id == receiver && row.snr_db == snr]
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-023C protected errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-023C protected successes do not reconcile")
        all(row.selection_reason in ALLOWED_SELECTION_REASONS for row in frames) ||
            error("AWGN-023C protected selection reason differs")
    end
    all(row.optimized_variables ==
        (row.algorithm_id == "profiled_cz" ? "C+z" : "C+W+z")
        for row in protected_traces) || error(
            "AWGN-023C optimized-variable label differs")
...
function write_csv(destination::AbstractString, rows, expected_header)
    isempty(rows) && error("AWGN-023C refuses an empty CSV")
    keys(first(rows)) == Symbol.(expected_header) || error(
        "AWGN-023C CSV schema differs")
...
function run_path(channel::Symbol, lane::Integer, config;
    ...
    println(channel, " hydrophone ", lane,
            " AWGN-023C N=1024 CP=64 rate=0.25 pilots=5/5 dc=14 ",
            "K=fill PFFT parts=4 capture=first 32.0 s frames=128 receivers=5")
