...
function aggregate_valid(path, channel, lane)
    ...
    expected = ("2048", "128", "0.25", "5", "10", "8", "4")
    ...
    all(rows) do row
        payload_per_frame = parse(Int, row[index["payload_bits_per_frame"]])
        ...
        payload_per_frame == 2921 && payload == 60 * payload_per_frame &&
...
function trace_valid(path, channel, lane)
    ...
    required = ("workload_id", "snr_db", "frame", "algorithm_id", "selection_reason",
                "bit_errors", "success", "decode_failure", "selected_iteration")
    all(haskey(index, key) for key in required) || return false
...
function pair_valid(aggregate_path, trace_path)
    ...
    for receiver in ("profiled_cz", "cwz_joint"), snr in SNRS
