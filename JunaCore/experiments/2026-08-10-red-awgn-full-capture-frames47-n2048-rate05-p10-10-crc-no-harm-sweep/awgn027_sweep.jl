...
const SHARED_REPOSITORY = get(
    ENV, "JUNA_AWGN027_SHARED_REPOSITORY",
    "/home/gabiel/Documents/GitHub/Juna-worktrees/awgn-results")
const RECEIVER_REPOSITORY = get(
    ENV, "JUNA_AWGN027_RECEIVER_REPOSITORY",
    "/home/gabiel/Documents/GitHub/Juna-worktrees/crc-no-harm-gradients")
const PROJECT_REPOSITORY = get(
    ENV, "JUNA_AWGN027_PROJECT_REPOSITORY",
    "/home/gabiel/Documents/GitHub/Juna-worktrees/src-001a-receiver-source")
const SHARED_EXPERIMENTS = joinpath(SHARED_REPOSITORY, "JunaCore", "experiments")
const PROJECT_EXPERIMENTS = joinpath(PROJECT_REPOSITORY, "JunaCore", "experiments")
...
const DECISION = "AWGN-027"
const FAMILY = "2026-08-10-red-awgn-full-capture-frames47-crc-no-harm"
const CONFIGURATIONS = [(
    id="$FAMILY-n2048-cp64-rate05-p10-10-dc14-kfill-pfft4",
    config=(nfft=2048, cp=64, code_rate=0.5,
...
const SNR_DB = 0:2:30
const FRAMES = 47
const CAPTURE_SECONDS = 47.78125
const SEED = 4
const PARTIAL_FFT_PARTS = 4
const EXPECTED_PAYLOAD_BITS_PER_FRAME = 3_296
...
const EXPECTED_ACTIVE_PROJECT = joinpath(
    PROJECT_EXPERIMENTS, "2026-08-04-red-snr-sweep", "Project.toml")
const AWGN023B_HARNESS_DIR = get(
    ENV, "JUNA_AWGN027_AWGN023B_HARNESS_DIR",
    joinpath(SHARED_EXPERIMENTS,
        "2026-08-10-red-awgn-full-capture-frames47-crc-no-harm-sweep"))
const AWGN023B_SCHEDULE_SOURCE =
    joinpath(AWGN023B_HARNESS_DIR, "awgn023b_sweep.jl")
const AWGN023B_SOURCE_CONTRACT =
    joinpath(AWGN023B_HARNESS_DIR, "source_contract.json")
const EXPECTED_AWGN023B_SCHEDULE_SOURCE_SHA256 =
    "6a995c62fbd938d69b7aa2c19f76e0bf618769b956fe764f64f19a60cb5d38ca"
const EXPECTED_AWGN023B_SOURCE_CONTRACT_SHA256 =
...
const EXPECTED_PROJECT_SHA256 = Dict(
    "Project.toml" =>
...
# Keep the AWGN-023B complete-capture placement policy while recomputing the
# evenly spaced positions for the N=2048 waveform geometry.
@eval B begin
    function _snapshot_positions(capture::ReplayCapture, packets::Integer,
        ...
        if count == 47
            ...
            positions == [1, 99, 197, 294, 392, 490, 588, 686, 783, 881, 979, 1077, 1175, 1272, 1370, 1468, 1566, 1664, 1761, 1859, 1957, 2055, 2153, 2250, 2348, 2446, 2544, 2642, 2740, 2837, 2935, 3033, 3131, 3229, 3326, 3424, 3522, 3620, 3718, 3815, 3913, 4011, 4109, 4207, 4304, 4402, 4500] || throw(ArgumentError(
                "N=2048 complete-capture positions differ"))
            return positions
        end
...
function _require_file_hash(path, expected, description)
    isfile(path) || error("AWGN-027 missing $description: $path")
    islink(path) && error("AWGN-027 refuses symlinked $description: $path")
    actual = file_sha256(path)
    actual == expected || error(
        "AWGN-027 $description content differs: expected $expected, got $actual")
end
...
function require_source_contract()
    ...
    package_root == realpath(joinpath(RECEIVER_REPOSITORY, "JunaCore")) ||
        error("AWGN-027 loaded JunaCore from $package_root")
...
function require_harness_contract()
    ...
    for (filename, expected) in EXPECTED_PROJECT_SHA256
        path = joinpath(
            PROJECT_EXPERIMENTS, "2026-08-04-red-snr-sweep", filename)
        _require_file_hash(path, expected, "active project $filename")
    end
    _require_file_hash(
        AWGN023B_SCHEDULE_SOURCE, EXPECTED_AWGN023B_SCHEDULE_SOURCE_SHA256,
        "AWGN-023B schedule source")
    _require_file_hash(
        AWGN023B_SOURCE_CONTRACT, EXPECTED_AWGN023B_SOURCE_CONTRACT_SHA256,
        "AWGN-023B source contract")
    nothing
end

function require_contract(; verify_data::Bool=true)
    length(CONFIGURATIONS) == 1 || error("AWGN-027 configuration count differs")
    only(CONFIGURATIONS).config == (
        nfft=2048, cp=64, code_rate=0.5,
        outer_spacing=10, inner_spacing=10,
        check_degree=14, horizon=0,
    ) || error("AWGN-027 configuration differs")
    EXPECTED_PAYLOAD_BITS_PER_FRAME == 3_296 &&
        EXPECTED_PAYLOAD_BITS_PER_POINT == 154_912 || error(
            "AWGN-027 payload contract differs")
    length(PATHS) == 12 || error("AWGN-027 path count differs")
    collect(SNR_DB) == collect(0:2:30) || error("AWGN-027 SNR grid differs")
    FRAMES == 47 && CAPTURE_SECONDS == 47.78125 && SEED == 4 || error(
        "AWGN-027 capture, frame, or seed contract differs")
    SNAPSHOT_INDICES == [1, 99, 197, 294, 392, 490, 588, 686, 783, 881, 979, 1077, 1175, 1272, 1370, 1468, 1566, 1664, 1761, 1859, 1957, 2055, 2153, 2250, 2348, 2446, 2544, 2642, 2740, 2837, 2935, 3033, 3131, 3229, 3326, 3424, 3522, 3620, 3718, 3815, 3913, 4011, 4109, 4207, 4304, 4402, 4500] || error(
        "AWGN-027 N=2048 replay schedule differs")
...
function _populate_capture_cache!(channel::Symbol, data_dir::AbstractString)
    ...
    isfile(file) || error("AWGN-027 missing measured capture: $file")
    islink(file) && error("AWGN-027 refuses symlinked measured capture: $file")
    ...
    for lane in 1:3
        ...
        _CAPTURE_CACHE[(String(data_dir), channel, lane)] = B.ReplayLane.ReplayCapture(
            ...
            cropped.receiver, replace(cropped.name, "first4s" => "full_capture"))
    end
    empty!(data)
    GC.gc()
end

function load_full_capture(channel::Symbol, lane::Integer;
...
function require_snapshot_contract(capture, config)
    config == only(CONFIGURATIONS).config || error(
        "AWGN-027 snapshot contract received another configuration")
    size(capture.h) == (768, 4_588) || error(
        "AWGN-027 cropped tap geometry differs")
    length(capture.phase) == 917_600 || error(
        "AWGN-027 cropped phase geometry differs")
    frame_samples = 8_320
    _, stop = B._capture_position_limit(
        capture, frame_samples + F.PASSBAND_MODEM_PADDING_SAMPLES, A.MODEM_FS)
    stop == 4_500 || error("AWGN-027 capture position stop differs: $stop")
    ...
    maximum(ends) <= CAPTURE_SECONDS || error(
        "AWGN-027 replay support exceeds the complete measured capture")
    last(ends) == 47.7734375 || error(
        "AWGN-027 final replay support differs")
    (
...
function _configure_protected_receiver(id::Symbol, capture, config)
    constructor = id === :profiled_cz ?
        ...
        JunaCore.JunaCrcConditionedJointCwzFrame.Modulation :
        error("AWGN-027 unknown protected receiver: $id")
    ...
    Modulations.isvalid(receiver, capture.fc, A.MODEM_FS) || error(
        "AWGN-027 public receiver $id is invalid after configuration")
    receiver
end
...
function evaluate(capture, channel_id, lane, config, snr_db;
    ...
    B._noise_override() === nothing || error(
        "AWGN-027 refuses a non-AWGN noise override")
    ...
    function decode(item, payload_bits, decoder_input, fc, fs)
        ...
        if id in (:ofdm_fec, :pfft, :lite)
            id === :lite && item.receiver.cz_crc_no_harm && error(
                "AWGN-027 Lite changed at decode boundary")
    ...
    function benchmark_sink(row)
        ...
        if id in (:profiled_cz, :cwz_joint)
            haskey(trace_bundles, id) || error(
                "AWGN-027 protected decode produced no no-harm trace")
    ...
    isempty(trace_bundles) || error(
        "AWGN-027 did not consume every protected trace")
    [(
...
function validate_path_rows(rows, frame_traces, protected_traces;
    ...
    for snr in expected_snrs, frame in 1:FRAMES
        ...
        length(paired) == length(ALGORITHMS) || error(
            "AWGN-027 paired workload receiver count differs")
        length(Set((row.workload_id, row.snapshot_index, row.payload_seed,
            ...
            "AWGN-027 receivers do not share one paired workload")
    end
    for receiver in RECEIVER_IDS, snr in expected_snrs
        ...
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-027 frame errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-027 frame successes do not reconcile")
    end
    for receiver in PROTECTED_RECEIVER_IDS, snr in expected_snrs
        ...
        frames = [row for row in protected_traces
                  if row.algorithm_id == receiver && row.snr_db == snr]
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-027 protected errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-027 protected successes do not reconcile")
        all(row.selection_reason in ALLOWED_SELECTION_REASONS for row in frames) ||
            error("AWGN-027 protected selection reason differs")
    end
    all(row.optimized_variables ==
        (row.algorithm_id == "profiled_cz" ? "C+z" : "C+W+z")
        for row in protected_traces) || error(
            "AWGN-027 optimized-variable label differs")
...
function write_csv(destination::AbstractString, rows, expected_header)
    isempty(rows) && error("AWGN-027 refuses an empty CSV")
    keys(first(rows)) == Symbol.(expected_header) || error(
        "AWGN-027 CSV schema differs")
...
function run_path(channel::Symbol, lane::Integer, config;
    ...
    capture = load_full_capture(channel, lane; data_dir)
    ...
    println(channel, " hydrophone ", lane,
            " AWGN-027 N=2048 CP=64 rate=0.5 pilots=10/10 dc=14 ",
            "K=fill PFFT parts=4 capture=complete 47.78125 s frames=47 receivers=5")
...
function probe_path(; channel::Symbol=:red1, lane::Integer=1,
    ...
    capture = load_full_capture(channel, lane; data_dir=DATA_DIR)
    rows = NamedTuple[]
