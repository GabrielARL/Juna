...
const DECISION = "AWGN-020"
const FAMILY = "2026-08-10-red-awgn-first8s-frames8-crc-no-harm"
...
const SNAPSHOT_INDICES = [1, 96, 190, 285, 385, 480, 574, 669]
const GENERIC_EIGHT_SNAPSHOT_INDICES = [1, 96, 192, 287, 383, 478, 574, 669]
const DATA_DIR = get(
...
# The first four positions are the original four-second campaign. The last
# four extend the observation without changing those original workloads.
@eval B begin
    function _snapshot_positions(capture::ReplayCapture, packets::Integer,
        ...
        if count == 8
            positions = [1, 96, 190, 285, 385, 480, 574, 669]
            last(positions) <= stop || throw(ArgumentError(
                "capture supports positions through $stop, needs 669"))
            return positions
        end
...
function _require_file_hash(path, expected, description)
    isfile(path) || error("AWGN-020 missing $description: $path")
    islink(path) && error("AWGN-020 refuses symlinked $description: $path")
    actual = file_sha256(path)
    actual == expected || error(
        "AWGN-020 $description content differs: expected $expected, got $actual")
end
...
function require_source_contract()
    ...
    package_root == realpath(joinpath(RECEIVER_REPOSITORY, "JunaCore")) ||
        error("AWGN-020 loaded JunaCore from $package_root")
...
function require_contract(; verify_data::Bool=true)
    length(CONFIGURATIONS) == 1 || error("AWGN-020 configuration count differs")
    length(PATHS) == 12 || error("AWGN-020 path count differs")
    collect(SNR_DB) == collect(0:2:30) || error("AWGN-020 SNR grid differs")
    FRAMES == 8 && CAPTURE_SECONDS == 8.0 && SEED == 4 || error(
        "AWGN-020 capture, frame, or seed contract differs")
    SNAPSHOT_INDICES == [1, 96, 190, 285, 385, 480, 574, 669] || error(
        "AWGN-020 nested replay positions differ")
...
function _populate_capture_cache!(channel::Symbol, data_dir::AbstractString)
    ...
    isfile(file) || error("AWGN-020 missing measured capture: $file")
    islink(file) && error("AWGN-020 refuses symlinked measured capture: $file")
    ...
    for lane in 1:3
        ...
        _CAPTURE_CACHE[(String(data_dir), channel, lane)] = B.ReplayLane.ReplayCapture(
            ...
            cropped.receiver, replace(cropped.name, "first4s" => "first8s"))
    end
    empty!(data)
    GC.gc()
end

function load_first8s_capture(channel::Symbol, lane::Integer;
...
function require_snapshot_contract(capture, config)
    config == only(CONFIGURATIONS).config || error(
        "AWGN-020 snapshot contract received another configuration")
    size(capture.h) == (768, 769) || error(
        "AWGN-020 cropped tap geometry differs")
    length(capture.phase) == 153_800 || error(
        "AWGN-020 cropped phase geometry differs")
    frame_samples = 9_536
    _, stop = B._capture_position_limit(
        capture, frame_samples + F.PASSBAND_MODEM_PADDING_SAMPLES, A.MODEM_FS)
    stop == 669 || error("AWGN-020 capture position stop differs: $stop")
    ...
    maximum(ends) <= CAPTURE_SECONDS || error(
        "AWGN-020 replay support exceeds the first eight seconds")
    (
...
function _configure_protected_receiver(id::Symbol, capture, config)
    constructor = id === :profiled_cz ?
        ...
        JunaCore.JunaCrcConditionedJointCwzFrame.Modulation :
        error("AWGN-020 unknown protected receiver: $id")
    ...
    Modulations.isvalid(receiver, capture.fc, A.MODEM_FS) || error(
        "AWGN-020 public receiver $id is invalid after configuration")
    receiver
end
...
function evaluate(capture, channel_id, lane, config, snr_db;
    ...
    B._noise_override() === nothing || error(
        "AWGN-020 refuses a non-AWGN noise override")
    ...
    function decode(item, payload_bits, decoder_input, fc, fs)
        ...
        if id in (:ofdm_fec, :pfft, :lite)
            id === :lite && item.receiver.cz_crc_no_harm && error(
                "AWGN-020 Lite changed at decode boundary")
    ...
    function benchmark_sink(row)
        ...
        if id in (:profiled_cz, :cwz_joint)
            haskey(trace_bundles, id) || error(
                "AWGN-020 protected decode produced no no-harm trace")
    ...
    isempty(trace_bundles) || error(
        "AWGN-020 did not consume every protected trace")
    [(
...
function validate_path_rows(rows, frame_traces, protected_traces;
    ...
    for snr in expected_snrs, frame in 1:FRAMES
        ...
        length(paired) == length(ALGORITHMS) || error(
            "AWGN-020 paired workload receiver count differs")
        length(Set((row.workload_id, row.snapshot_index, row.payload_seed,
            ...
            "AWGN-020 receivers do not share one paired workload")
    end
    for receiver in RECEIVER_IDS, snr in expected_snrs
        ...
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-020 frame errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-020 frame successes do not reconcile")
    end
    for receiver in PROTECTED_RECEIVER_IDS, snr in expected_snrs
        ...
        frames = [row for row in protected_traces
                  if row.algorithm_id == receiver && row.snr_db == snr]
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-020 protected errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-020 protected successes do not reconcile")
        all(row.selection_reason in ALLOWED_SELECTION_REASONS for row in frames) ||
            error("AWGN-020 protected selection reason differs")
    end
    all(row.optimized_variables ==
        (row.algorithm_id == "profiled_cz" ? "C+z" : "C+W+z")
        for row in protected_traces) || error(
            "AWGN-020 optimized-variable label differs")
...
function write_csv(destination::AbstractString, rows, expected_header)
    isempty(rows) && error("AWGN-020 refuses an empty CSV")
    keys(first(rows)) == Symbol.(expected_header) || error(
        "AWGN-020 CSV schema differs")
...
function run_path(channel::Symbol, lane::Integer, config;
    ...
    capture = load_first8s_capture(channel, lane; data_dir)
    ...
    println(channel, " hydrophone ", lane,
            " AWGN-020 N=1024 CP=64 rate=0.25 pilots=5/5 dc=14 ",
            "K=fill PFFT parts=4 capture=first 8.0 s frames=8 receivers=5")
...
function probe_path(; channel::Symbol=:red1, lane::Integer=1,
    ...
    capture = load_first8s_capture(channel, lane; data_dir=DATA_DIR)
    rows = NamedTuple[]
