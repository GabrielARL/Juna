#!/usr/bin/env python3
"""Approved AWGN-020 geometry and result-shape contract."""

import re


FAMILY = "2026-08-10-red-awgn-first8s-frames8-crc-no-harm"
CAMPAIGN = "AWGN-020"
EXPECTED_IDS = (
...
FRAME_TRACE_ROWS_PER_PATH = 640
PROTECTED_TRACE_ROWS_PER_PATH = 256


class MatrixContractError(RuntimeError):
    """The approved AWGN-020 matrix shape was not met."""
...
def parse_geometry(experiment_id):
    ...
    if match is None:
        raise MatrixContractError(
            "unexpected AWGN-020 experiment ID: " + experiment_id
        )
...
def validate_source_contract(payload):
    ...
    if not re.fullmatch(
        r"[0-9a-f]{64}", receiver_source.get("tracked_source_diff_sha256", "")
    ):
...
if __name__ == "__main__":
    validate_ids()
    print(
        "AWGN-020 MATRIX CONTRACT PASS: 1 configuration, 12 paths, "
        "5 receivers, 0:2:30 dB, first 8 seconds, 8 frames, PFFT=4"
    )
