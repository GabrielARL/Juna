#!/usr/bin/env python3
"""Approved AWGN-021 geometry and result-shape contract."""

import re


FAMILY = "2026-08-10-red-awgn-first16s-frames16-crc-no-harm"
CAMPAIGN = "AWGN-021"
EXPECTED_IDS = (
...
FRAME_TRACE_ROWS_PER_PATH = 1280
PROTECTED_TRACE_ROWS_PER_PATH = 512


class MatrixContractError(RuntimeError):
    """The approved AWGN-021 matrix shape was not met."""
...
def parse_geometry(experiment_id):
    ...
    if match is None:
        raise MatrixContractError(
            "unexpected AWGN-021 experiment ID: " + experiment_id
        )
...
if __name__ == "__main__":
    validate_ids()
    print(
        "AWGN-021 MATRIX CONTRACT PASS: 1 configuration, 12 paths, "
        "5 receivers, 0:2:30 dB, first 16 seconds, 16 frames, PFFT=4"
    )
