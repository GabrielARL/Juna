#!/usr/bin/env python3
"""Independent on-disk acceptance checks for one AWGN-022 result."""
...
def require(condition, message):
    if not condition:
        raise builder.ResultContractError(
            "AWGN-022 validation failed: " + message)
...
def validate(experiment_dir,
             reference_results=builder.AWGN021_REFERENCE_RESULTS,
             reference_hashes=builder.AWGN021_REFERENCE_HASHES):
    ...
    require("AWGN-022: BER versus SNR" in page,
            "approved title is absent")
    require("first thirty-two seconds" in normalized_page,
            "approved capture statement is absent")
    require("first sixteen replay windows are unchanged" in normalized_page,
            "approved nesting statement is absent")
    require("sixteen later replay windows extend the observation to thirty-two seconds"
    ...
    print(
        "VALID AWGN-022: 12/12 paths, 960 aggregates, 30720 frame traces, "
        "12288 protected traces, 12 panels, 60 series, payload=1616"
    )
    return manifest
...
def main():
    ...
    parser.add_argument("--reference-results",
                        default=builder.AWGN021_REFERENCE_RESULTS)
