#!/usr/bin/env python3
"""Validate, combine, and render the approved AWGN-022 result."""
...
SOURCE_BASENAME = "red_snr_sweep_awgn_first32s_frames32_configuration.csv"
OUTPUT_BASENAME = "red_snr_sweep_awgn_first32s_frames32.csv"
PATH_CONTRACT_BASENAME = "awgn022_path_contract.txt"
AWGN021_EXPERIMENT_ID = (
    "2026-08-10-red-awgn-first16s-frames16-crc-no-harm-"
    "n1024-cp64-rate025-p5-5-dc14-kfill-pfft4"
)
AWGN021_REFERENCE_RESULTS = os.path.join(
    "/home/gabiel/Documents/GitHub/Juna-worktrees/awgn-results/"
    "JunaCore/experiments",
    AWGN021_EXPERIMENT_ID,
    "results",
)
AWGN021_REFERENCE_HASHES = {
    "aggregate": "d41580ad033d448140b3f5045f24a3dad677d5cd18c0c0692cda3d05c74f9d20",
    "manifest": "779272a2c04241b27316396ea1b1efcd80415c64b7da5158063ea791ca0a4128",
    "view": "cac31fca4e7ec840b16069ffbab69cc9629324a97ab24fa4466d74f2705fc5cf",
}
AWGN021_ARTIFACTS = {
    "aggregate": "red_snr_sweep_awgn_first16s_frames16.csv",
...
class ResultContractError(RuntimeError):
    """One measured AWGN-022 artifact violated the approved contract."""


def require(condition, message):
    if not condition:
        raise ResultContractError("AWGN-022 result contract failed: " + message)
...
def reference_artifact_hashes(results_dir):
    return {
        ...
        for name, filename in AWGN021_ARTIFACTS.items()
    }
...
def _path_contract_record(results_dir, experiment_id, channel, hydrophone,
    ...
    expected = (
        ...
        ("runner_sha256", file_digest(os.path.join(HERE, "run_awgn022.jl"))),
        ("sweep_sha256", file_digest(os.path.join(HERE, "awgn022_sweep.jl"))),
        ("aggregate_rows", "80"),
        ("frame_trace_rows", "2560"),
        ("protected_trace_rows", "1024"),
...
def _validate_aggregate(rows, source, channel, hydrophone,
    ...
    for row in rows:
        ...
        require(row["seed"] == "4" and row["frames"] == "32",
        ...
        require(int(row["payload_bits_per_frame"]) == 1616
                and int(row["payload_bits"]) == 32 * 1616,
...
def _validate_frame_traces(rows, source):
    require(len(rows) == 2560,
            f"expected 2560 frame traces, found {len(rows)}: {source}")
    require({row["algorithm_id"] for row in rows} == EXPECTED_RECEIVERS,
    ...
    keys = {(row["algorithm_id"], float(row["snr_db"]), int(row["frame"]))
            for row in rows}
    require(len(keys) == 2560, "frame-trace keys are duplicated: " + source)
    for row in rows:
        frame = int(row["frame"])
        require(1 <= frame <= 32, "frame index differs: " + source)
...
def _validate_protected_traces(rows, source, frame_rows):
    require(len(rows) == 1024,
            f"expected 1024 protected traces, found {len(rows)}: {source}")
    require({row["algorithm_id"] for row in rows} == PROTECTED_RECEIVERS,
            "protected receiver set differs: " + source)
    keys = {(row["algorithm_id"], float(row["snr_db"]), int(row["frame"]))
            for row in rows}
    require(len(keys) == 1024, "protected keys are duplicated: " + source)
    frame_index = {
...
def _reconcile(aggregates, frame_rows, source, frame_count=32):
...
def _awgn021_reference(candidate_frames, candidate_protected,
                       reference_results, reference_hashes):
    require(os.path.isdir(reference_results), "missing AWGN-021 reference results")
    for filename in AWGN021_ARTIFACTS.values():
        path = os.path.join(reference_results, filename)
        require(os.path.isfile(path) and not os.path.islink(path),
                "missing AWGN-021 reference artifact: " + filename)
    ...
    if reference_hashes is not None:
        for name, expected in reference_hashes.items():
            require(actual_hashes.get(name) == expected,
                    f"AWGN-021 {name} hash differs")
    manifest_path = os.path.join(reference_results, AWGN021_ARTIFACTS["manifest"])
    with open(manifest_path, encoding="utf-8") as handle:
        manifest = json.load(handle)
    require(manifest.get("decision") == "AWGN-021",
            "AWGN-021 manifest decision differs")
    require(manifest.get("experiment_id") == AWGN021_EXPERIMENT_ID,
            "AWGN-021 manifest experiment differs")
    require(manifest.get("row_count") == 960
            and manifest.get("frame_trace_row_count") == 15360
            and manifest.get("protected_trace_row_count") == 6144,
            "AWGN-021 manifest counts differ")
    frame_records = _reference_records(manifest, "frame_trace_sources", 1280)
    protected_records = _reference_records(
        manifest, "protected_trace_sources", 512)
    matched_frames = 0
    matched_protected = 0
    frame_evidence = []
    protected_evidence = []
    ...
    for channel, hydrophone in EXPECTED_PATHS:
        ...
        require(frame_name in frame_records,
                "AWGN-021 frame evidence path differs: " + frame_name)
        require(protected_name in protected_records,
                "AWGN-021 protected evidence path differs: " + protected_name)
        ...
        require(file_digest(frame_path) == frame_record["sha256"],
                "AWGN-021 frame evidence hash differs: " + frame_name)
        require(file_digest(protected_path) == protected_record["sha256"],
                "AWGN-021 protected evidence hash differs: " + protected_name)
        columns, reference_frames = load_csv(reference_results, frame_name)
        require(columns == FRAME_TRACE_COLUMNS and len(reference_frames) == 1280,
                "AWGN-021 frame evidence schema differs: " + frame_name)
        columns, reference_protected = load_csv(reference_results, protected_name)
        require(columns == PROTECTED_TRACE_COLUMNS
                and len(reference_protected) == 512,
                "AWGN-021 protected evidence schema differs: " + protected_name)
        _compare_reference_rows(
            candidate_frames[path_key], reference_frames,
            REFERENCE_FRAME_FIELDS, "AWGN-021 first-sixteen frame")
        _compare_reference_rows(
            candidate_protected[path_key], reference_protected,
            REFERENCE_PROTECTED_FIELDS, "AWGN-021 protected")
    ...
    aggregate_name = AWGN021_ARTIFACTS["aggregate"]
            ...
            for snr in EXPECTED_SNRS:
                ...
                require(len(selected) == 16
                        ...
                        and int(aggregate["successful_frames"]) == sum(
                            _bool(row, "success", "AWGN-021") for row in selected)
                        and int(aggregate["decode_failures"]) == sum(
                            _bool(row, "decode_failure", "AWGN-021")
                            for row in selected),
                        "AWGN-021 aggregate/frame outcomes differ")
    return {
        "decision": "AWGN-021",
        "experiment_id": AWGN021_EXPERIMENT_ID,
        "aggregate": {
            "path": aggregate_name, "sha256": actual_hashes["aggregate"],
            "rows": 960,
        },
        "manifest": {
            "path": AWGN021_ARTIFACTS["manifest"],
            "sha256": actual_hashes["manifest"],
        },
        "view": {
            "path": AWGN021_ARTIFACTS["view"],
...
def collect(experiment_dir, reference_results=AWGN021_REFERENCE_RESULTS,
            reference_hashes=AWGN021_REFERENCE_HASHES):
    ...
    require(sum(item["rows"] for item in frame_sources) == 30720,
            "combined frame-trace row count differs")
    require(sum(item["rows"] for item in protected_sources) == 12288,
            "combined protected row count differs")
    require(bands == {16}, "partial-FFT band count differs")
    reference = _awgn021_reference(
...
def load_renderer(receiver_ids):
    ...
    spec = importlib.util.spec_from_file_location("awgn022_sweep_view", path)
...
<p><b>Frames.</b> Each SNR point uses thirty-two approximately non-overlapping frames
whose complete replay windows fit inside the first thirty-two seconds. The first
sixteen replay windows are unchanged from the sixteen-second results; sixteen later
replay windows extend the observation to thirty-two seconds. All five receivers use
...
def _family_source_hashes():
    names = (
        "awgn022_contract_test.jl", "awgn022_sweep.jl", "run_awgn022.jl",
...
def build(experiment_dir, reference_results=AWGN021_REFERENCE_RESULTS,
          reference_hashes=AWGN021_REFERENCE_HASHES):
    ...
    manifest = {
        ...
        "frame_window_selection": (
            "thirty-two approximately non-overlapping complete replay windows; "
            "the first sixteen are unchanged from the sixteen-second result"),
        ...
        "result_scope": (
            "BER versus SNR, first 32 s, 32 frames per point, seed 4"),
        "row_count": 960,
    ...
    _atomic_text(os.path.join(results_dir, "results_view.html"),
                 render_page(rows))
    print("BUILT AWGN-022: 960 rows, 30720 frame traces, 12288 protected "
...
def main():
    ...
    parser.add_argument("--reference-results", default=AWGN021_REFERENCE_RESULTS)
