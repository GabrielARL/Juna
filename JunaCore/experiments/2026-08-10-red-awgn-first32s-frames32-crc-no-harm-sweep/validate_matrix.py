#!/usr/bin/env python3
"""Validate the complete one-configuration AWGN-022 campaign."""
...
def main():
    experiments = os.environ.get(
        "JUNA_AWGN022_OUTPUT_EXPERIMENTS",
        "/home/gabiel/Documents/GitHub/Juna-worktrees/awgn-022-first32s/"
    ...
    if sum(item["row_count"] for item in manifests) != 960:
        raise SystemExit("AWGN-022 matrix aggregate count differs")
    if sum(item["frame_trace_row_count"] for item in manifests) != 30720:
        raise SystemExit("AWGN-022 matrix frame-trace count differs")
    if sum(item["protected_trace_row_count"] for item in manifests) != 12288:
        raise SystemExit("AWGN-022 matrix protected-trace count differs")
    print(
        "AWGN-022 MATRIX VALID: 1/1 configuration, 12/12 paths, "
