#!/usr/bin/env python3
"""Synthetic contract tests for the AWGN-022 result pipeline."""
...
def _aggregate(channel, hydrophone, receiver, snr, frames, frame_count=32):
    ...
    return {
        ...
        "capture_tap_snapshots": "3073" if frame_count == 32 else "1537",
        "capture_phase_samples": "614600" if frame_count == 32 else "307400",
    }


def _write_path_contract(run_dir, experiment_id, channel, hydrophone):
    values = (
        ("campaign", "AWGN-022"), ("experiment_id", experiment_id),
        ...
        ("runner_sha256", builder.file_digest(
            os.path.join(builder.HERE, "run_awgn022.jl"))),
        ("sweep_sha256", builder.file_digest(
            os.path.join(builder.HERE, "awgn022_sweep.jl"))),
...
def _reference_fixture(root, path_rows):
    results = os.path.join(root, "awgn021-reference", "results")
    ...
    for channel, hydrophone in builder.EXPECTED_PATHS:
        ...
        frame_sources.append({
            ...
            "rows": 1280, "sha256": builder.file_digest(frame_path),
        })
        protected_sources.append({
            "path": f"runs/{channel}_hydrophone{hydrophone}/{protected_name}",
            "rows": 512, "sha256": builder.file_digest(protected_path),
        })
    aggregate_path = os.path.join(
        results, "red_snr_sweep_awgn_first16s_frames16.csv")
    _write_csv(aggregate_path, builder.AGGREGATE_COLUMNS, aggregates)
    manifest = {
        "decision": "AWGN-021",
        "experiment_id": builder.AWGN021_EXPERIMENT_ID,
        "row_count": 960, "frame_trace_row_count": 15360,
    ...
    with open(os.path.join(results, "results_view.html"),
              "w", encoding="utf-8") as handle:
        handle.write("<title>AWGN-021: BER versus SNR</title>\n")
    return results
...
class Awgn022ResultsContract(unittest.TestCase):
    def test_page022a_reader_wording_is_exact(self):
        path = os.path.join(
            builder.AWGN021_REFERENCE_RESULTS,
            builder.AWGN021_ARTIFACTS["aggregate"],
        )
        ...
        self.assertEqual(page.count("AWGN-022: BER versus SNR"), 2)

    def test_retained_awgn021_artifacts_and_trace_evidence_are_pinned(self):
        results = builder.AWGN021_REFERENCE_RESULTS
        self.assertEqual(
            builder.reference_artifact_hashes(results),
            builder.AWGN021_REFERENCE_HASHES,
        )
        with open(os.path.join(results, "results_manifest.json"),
                  encoding="utf-8") as handle:
            manifest = json.load(handle)
        for field, rows in (("frame_trace_sources", 1280),
                            ("protected_trace_sources", 512)):
    ...
    def test_build_and_validate_synthetic_complete_result(self):
        with tempfile.TemporaryDirectory(prefix="awgn022-results-") as root:
            ...
            with open(os.path.join(
                    ...
                    encoding="utf-8") as handle:
                manifest = json.load(handle)
            self.assertEqual(manifest["decision"], "AWGN-022")
            self.assertEqual(manifest["row_count"], 960)
            self.assertEqual(manifest["frame_trace_row_count"], 30720)
            self.assertEqual(manifest["protected_trace_row_count"], 12288)
            self.assertEqual(manifest["panel_count"], 12)
            self.assertEqual(manifest["series_count"], 60)
            self.assertEqual(
                manifest["awgn021_reference"]["matched_frame_rows"], 15360)
            self.assertEqual(
                manifest["awgn021_reference"]["matched_protected_rows"], 6144)

    def test_first_sixteen_replay_drift_is_rejected(self):
        with tempfile.TemporaryDirectory(prefix="awgn022-results-") as root:
            experiment, reference = _fixture(root)
            path = os.path.join(
            ...
            with self.assertRaisesRegex(
                    builder.ResultContractError, "AWGN-021 first-sixteen frame"):
                builder.collect(experiment, reference_results=reference,
                                reference_hashes=None)

    def test_first_sixteen_protected_selection_drift_is_rejected(self):
        with tempfile.TemporaryDirectory(prefix="awgn022-results-") as root:
            experiment, reference = _fixture(root)
            path = os.path.join(
                experiment, "results",
            ...
            _refresh_path_contract(experiment)
            with self.assertRaisesRegex(
                    builder.ResultContractError, "AWGN-021 protected"):
                builder.collect(experiment, reference_results=reference,
                                reference_hashes=None)
    ...
    def test_reference_artifact_drift_is_rejected(self):
        with tempfile.TemporaryDirectory(prefix="awgn022-results-") as root:
            ...
            with open(os.path.join(reference, "results_view.html"),
                ...
                handle.write("drift")
            with self.assertRaisesRegex(
                    builder.ResultContractError, "AWGN-021 view hash"):
                builder.collect(experiment, reference_results=reference,
