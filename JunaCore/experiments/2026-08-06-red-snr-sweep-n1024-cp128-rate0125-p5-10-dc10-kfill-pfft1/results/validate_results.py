#!/usr/bin/env python3
"""Validate N=1024, CP=128, rate=0.125, pilots=5/10, K=fill, PFFT=1."""
...
EXPERIMENT_ID = (
    "2026-08-06-red-snr-sweep-n1024-cp128-rate0125-p5-10-dc10-kfill-pfft1")
...
EXPECTED_GEOMETRY = ("1024", "128", "0.125", "5", "10", "10", "0")
EXPECTED_PAYLOAD_BITS_PER_FRAME = 718
...
def require(condition, message):
    if not condition:
        raise SystemExit(
            "N1024 CP128 rate-0.125 pilots-5/10 K=fill validation failed: "
            + message)
...
def main():
    ...
    for row in rows:
        ...
        require(int(row["payload_bits_per_frame"]) ==
                ...
                "an aggregate row has the wrong payload capacity")
        require(int(row["partial_fft_parts"]) == EXPECTED_PARTIAL_FFT_PARTS,
                "an aggregate row has the wrong partial-FFT part count")
    ...
    for channel, lane in EXPECTED_PATHS:
        ...
        trace_name_value = trace_name(channel, lane)
        trace = read_csv(os.path.join(HERE, trace_name_value))
        trace_paths.add(trace_name_value)
        ...
        require({int(row["partial_fft_parts"]) for row in trace} == {
                EXPECTED_PARTIAL_FFT_PARTS},
                f"{channel} hydrophone {lane} partial-FFT part count differs")
        require(all(
            row["workload_id"] ==
            f"{channel}:lane{lane}:snr{float(row['snr_db'])}:seed4:"
            f"frame{int(row['frame'])}"
            for row in trace
        ), f"{channel} hydrophone {lane} trace provenance differs")
        ...
        for receiver in PROFILED_RECEIVERS:
            ...
            for snr in EXPECTED_SNRS:
                ...
                require(len(frames) == 60, "trace frame count differs")
    ...
    require(manifest["partial_fft_parts"] == EXPECTED_PARTIAL_FFT_PARTS,
            "manifest partial-FFT part count differs")
    ...
    require({source["path"] for source in manifest["trace_sources"]} ==
            trace_paths, "manifest trace source paths differ")
    ...
    require(page.count("K=fill") == 12,
            "not every panel displays K=fill")
    ...
    caption = ("N=1024 · CP=128 · rate=0.125 · pilots=5/10 · dc=10 · "
               "K=fill · PFFT parts=1")
    for channel, lane in EXPECTED_PATHS:
    ...
    view_builder = os.path.join(
        ...
        "build_sweep_view.py")
    namespace = {"__file__": view_builder,
                 "__name__": "_n1024_cp128_rate0125_p5_10_view"}
    ...
    print("N1024 CP128 rate-0.125 pilots-5/10 K=fill PFFT=1 validation passed: "
