#!/usr/bin/env python3
"""Validate the requested-configuration twelve-panel SNR result."""
...
EXPECTED_GEOMETRY = ("512", "16", "0.5", "3", "4", "10", "0")
...
OLD_HASHES = {
    "red_snr_sweep_uwa_noise.csv":
        "2c219339ff4ec7db44c9f5d27dc123d1d23927e1a2f296228730488b9da6dc7c",
    "results_manifest.json":
        "7a29d148cd68313ad229c2071c0adfce7613418f22bee4673092ecec04e79d9b",
    "results_view.html":
        "0a72e27cf6d24186b00875c05560b900aa02545dd21275ffc4f6c169ee117d41",
}


def require(condition, message):
    if not condition:
        raise SystemExit("fixed-configuration validation failed: " + message)
...
def main():
    ...
    require(trace_total == 23040, f"expected 23040 trace rows, found {trace_total}")
    ...
    require(manifest["geometry_display"]["horizon"] == "fill",
    ...
    caption = "N=512 · CP=16 · rate=0.5 · pilots=3/4 · dc=10 · K=fill"
    for channel, lane in EXPECTED_PATHS:
    ...
    old_results = os.path.join(os.path.dirname(os.path.dirname(HERE)),
                               "2026-08-04-red-snr-sweep", "results")
    for name, expected in OLD_HASHES.items():
        require(sha256(os.path.join(old_results, name)) == expected,
                f"original result changed: {name}")

    print("fixed-configuration validation passed: 960 aggregate rows, "
