#!/usr/bin/env python3
"""Approved AWGN-024 geometry and result-shape contract."""

import re


FAMILY = "2026-08-10-red-awgn-first32s-frames32-rate05-crc-no-harm"
CAMPAIGN = "AWGN-024"
EXPECTED_IDS = (
    f"{FAMILY}-n1024-cp64-rate05-p5-5-dc14-kfill-pfft4",
)
ID_RE = re.compile(
    rf"^{re.escape(FAMILY)}-n(?P<nfft>1024)-cp(?P<cp>64)-"
    r"rate(?P<rate>05)-p(?P<outer>5)-(?P<inner>5)-"
...
class MatrixContractError(RuntimeError):
    """The approved AWGN-024 matrix shape was not met."""
...
def parse_geometry(experiment_id):
    ...
    if match is None:
        raise MatrixContractError(
            "unexpected AWGN-024 experiment ID: " + experiment_id
        )
...
def validate_source_contract(payload):
    ...
    if payload.get("payload_bits_per_frame") != 3248:
...
if __name__ == "__main__":
    validate_ids()
    print(
        "AWGN-024 MATRIX CONTRACT PASS: 1 configuration, 12 paths, "
