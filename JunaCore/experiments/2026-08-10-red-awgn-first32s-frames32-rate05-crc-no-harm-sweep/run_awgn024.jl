...
using .Awgn024Sweep
using Dates
const A = Awgn024Sweep

const EXPERIMENTS = get(
    ENV, "JUNA_AWGN024_OUTPUT_EXPERIMENTS", dirname(@__DIR__))
const FIRST_EXPERIMENT = joinpath(EXPERIMENTS, first(A.CONFIGURATIONS).id)
const CAMPAIGN_LOG = joinpath(FIRST_EXPERIMENT, "awgn024_sweep.log")
const SWEEP_SOURCE = joinpath(@__DIR__, "awgn024_sweep.jl")
...
function result_paths(experiment_dir::AbstractString, channel::Symbol,
    ...
    (
        ...
        contract=joinpath(
            run_dir, "awgn024_path_contract.txt" * suffix),
    )
end
...
function run_configuration(spec)
    ...
    for (channel, hydrophone) in A.PATHS
        ...
        if content_valid(spec, channel, hydrophone, final)
            marker("SKIP_VALID AWGN-024 $(spec.id) $(channel) hydrophone " *
                   string(hydrophone))
            continue
        end
        staged = result_paths(
            experiment_dir, channel, hydrophone; partial=true)
        marker("PATH_START AWGN-024 $(spec.id) $(channel) hydrophone " *
               string(hydrophone))
        ...
        data_shape_valid(staged) || error(
            "AWGN-024 staged path files failed exact shape contract")
        write_path_contract(
            staged.contract, spec, channel, hydrophone, staged)
        promote_path(staged, final)
        content_valid(spec, channel, hydrophone, final) || error(
            "AWGN-024 final path content contract differs")
        marker("PATH_VALID AWGN-024 $(spec.id) $(channel) hydrophone " *
               string(hydrophone))
        GC.gc()
    end
    ...
    marker("CONFIGURATION_COMPLETE AWGN-024 $(spec.id)")
end

function main()
    if ARGS == ["contract"]
        A.require_contract(; verify_data=false)
        println("AWGN_024_CONTRACT_VALID configurations=1 paths=12")
        return
    end
    marker("AWGN_024_QUEUE_START")
    ...
    length(targets) == (isempty(requested) ? length(A.CONFIGURATIONS) :
        ...
        "AWGN-024 received an unknown or duplicate experiment ID")
    marker("AWGN_024_COMPUTE_START")
    foreach(run_configuration, targets)
    if isempty(requested)
        matrix_validator = joinpath(@__DIR__, "validate_matrix.py")
        run(`python3 $matrix_validator`)
    end
    marker("AWGN_024_COMPUTE_COMPLETE")
    println("AWGN_024_MATRIX_COMPLETE configurations=", length(targets),
