#!/usr/bin/env python3
"""Validate, combine, and render the approved AWGN-024 result."""
...
PATH_CONTRACT_BASENAME = "awgn024_path_contract.txt"
...
class ResultContractError(RuntimeError):
    """One measured AWGN-024 artifact violated the approved contract."""


def require(condition, message):
    if not condition:
        raise ResultContractError("AWGN-024 result contract failed: " + message)
...
def _path_contract_record(results_dir, experiment_id, channel, hydrophone,
    ...
    expected = (
        ...
        ("runner_sha256", file_digest(os.path.join(HERE, "run_awgn024.jl"))),
        ("sweep_sha256", file_digest(os.path.join(HERE, "awgn024_sweep.jl"))),
        ("aggregate_rows", "80"),
...
def _validate_aggregate(rows, source, channel, hydrophone,
    ...
    for row in rows:
        ...
        require(int(row["payload_bits_per_frame"]) == PAYLOAD_BITS_PER_FRAME
                and int(row["payload_bits"]) == 32 * PAYLOAD_BITS_PER_FRAME,
...
def _awgn022_reference(_candidate_frames, _candidate_protected,
    ...
    return {
        ...
        "aggregate": {
            "path": AWGN022_ARTIFACTS["aggregate"],
            "sha256": actual_hashes["aggregate"],
            "rows": 960,
        },
        "manifest": {
        ...
        "candidate_comparison": "not applied because the code rate differs",
    }
...
def load_renderer(receiver_ids):
    ...
    spec = importlib.util.spec_from_file_location("awgn024_sweep_view", path)
...
def _family_source_hashes():
    names = (
        "awgn024_contract_test.jl", "awgn024_sweep.jl", "run_awgn024.jl",
...
def build(experiment_dir, reference_results=AWGN022_REFERENCE_RESULTS,
    ...
    _atomic_text(os.path.join(results_dir, "results_view.html"),
                 render_page(rows))
    print("BUILT AWGN-024: 960 rows, 30720 frame traces, 12288 protected "
          "traces, 12 panels, 60 series, payload=3248")
