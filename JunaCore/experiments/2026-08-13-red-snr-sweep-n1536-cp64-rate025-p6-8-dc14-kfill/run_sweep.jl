#!/usr/bin/env julia
# CL-151: ambient-noise sweep at N=1536, the geometry of the joint paper's
# Fig. 6, under the uwa-channels alpha-stable noise model and the ungated
# gradient arms.  Adapted from
# 2026-08-05-red-snr-sweep-n2048-cp16-rate025-p3-5-dc10-k4/run_sweep.jl;
# partial-FFT parts stay at the harness default of four, matching the
# suffix-free members of this family.
...
const DATA_DIR = normpath(joinpath(
    homedir(), "Documents", "GitHub", "Juna", "JunaCore", "experiments",
...
const EXPECTED_GEOMETRY = ("1536", "64", "0.25", "6", "8", "14", "0")
const EXPECTED_PAYLOAD_BITS_PER_FRAME = 1653
const RESULT_LABEL = "configuration"
const ALL_PATHS = [(Symbol("red$channel"), lane)
...
function selected_paths()
    raw = get(ENV, "SWEEP_PATHS", "")
    isempty(raw) && return ALL_PATHS
    wanted = Set(split(raw, ','; keepempty=false))
    [(channel, lane) for (channel, lane) in ALL_PATHS
     if "$(channel):$(lane)" in wanted]
end
...
function main()
    ...
    isdir(DATA_DIR) || error("capture data directory missing: $(DATA_DIR)")
    for (channel, lane) in selected_paths()
