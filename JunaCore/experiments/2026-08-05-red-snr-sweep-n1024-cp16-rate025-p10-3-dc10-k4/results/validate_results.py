#!/usr/bin/env python3
"""Validate the N=1024, rate=0.25, pilots=10/3, K=4 twelve-path result."""
...
EXPECTED_GEOMETRY = ("1024", "16", "0.25", "10", "3", "10", "4")
EXPECTED_PAYLOAD_BITS_PER_FRAME = 1210
...
def require(condition, message):
    if not condition:
        raise SystemExit(
            "N1024 CP16 rate-0.25 pilots-10/3 K=4 validation failed: "
            + message)
...
def main():
    ...
    caption = "N=1024 · CP=16 · rate=0.25 · pilots=10/3 · dc=10 · K=4"
    for channel, lane in EXPECTED_PATHS:
    ...
    view_builder = os.path.join(
        ...
        "build_sweep_view.py")
    namespace = {"__file__": view_builder, "__name__": "_n1024_p10_3_view"}
    ...
    print("N1024 CP16 rate-0.25 pilots-10/3 K=4 validation passed: "
