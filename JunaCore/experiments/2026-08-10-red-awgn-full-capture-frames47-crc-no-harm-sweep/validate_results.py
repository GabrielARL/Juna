#!/usr/bin/env python3
"""Independent on-disk acceptance checks for one AWGN-023B result."""
...
def require(condition, message):
    if not condition:
        raise builder.ResultContractError(
            "AWGN-023B validation failed: " + message)
...
def validate(experiment_dir,
    ...
    require("AWGN-023B: BER versus SNR" in page,
            "approved title is absent")
    require("complete measured red replay capture" in normalized_page,
            "approved capture statement is absent")
    require("first thirty-two replay windows are unchanged" in normalized_page,
            "approved nesting statement is absent")
    require("fifteen later replay windows extend the measured observation to 47.78125 seconds"
    ...
    print(
        "VALID AWGN-023B: 12/12 paths, 960 aggregates, 45120 frame traces, "
        "18048 protected traces, 12 panels, 60 series, payload=1616"
    )
    return manifest
