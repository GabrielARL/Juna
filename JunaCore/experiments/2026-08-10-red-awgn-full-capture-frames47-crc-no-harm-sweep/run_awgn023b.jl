...
using .Awgn023bSweep
using Dates
const A = Awgn023bSweep

const EXPERIMENTS = get(
    ENV, "JUNA_AWGN023B_OUTPUT_EXPERIMENTS", dirname(@__DIR__))
const FIRST_EXPERIMENT = joinpath(EXPERIMENTS, first(A.CONFIGURATIONS).id)
const CAMPAIGN_LOG = joinpath(FIRST_EXPERIMENT, "awgn023b_sweep.log")
const SWEEP_SOURCE = joinpath(@__DIR__, "awgn023b_sweep.jl")
...
function result_paths(experiment_dir::AbstractString, channel::Symbol,
    ...
    (
        ...
        contract=joinpath(
            run_dir, "awgn023b_path_contract.txt" * suffix),
    )
end
...
function run_configuration(spec)
    ...
    for (channel, hydrophone) in A.PATHS
        ...
        if content_valid(spec, channel, hydrophone, final)
            marker("SKIP_VALID AWGN-023B $(spec.id) $(channel) hydrophone " *
                   string(hydrophone))
            continue
        end
        staged = result_paths(
            experiment_dir, channel, hydrophone; partial=true)
        marker("PATH_START AWGN-023B $(spec.id) $(channel) hydrophone " *
               string(hydrophone))
        ...
        data_shape_valid(staged) || error(
            "AWGN-023B staged path files failed exact shape contract")
        write_path_contract(
            staged.contract, spec, channel, hydrophone, staged)
        promote_path(staged, final)
        content_valid(spec, channel, hydrophone, final) || error(
            "AWGN-023B final path content contract differs")
        marker("PATH_VALID AWGN-023B $(spec.id) $(channel) hydrophone " *
               string(hydrophone))
        GC.gc()
    end
    ...
    marker("CONFIGURATION_COMPLETE AWGN-023B $(spec.id)")
end

function main()
    if ARGS == ["contract"]
        A.require_contract(; verify_data=false)
        println("AWGN_023B_CONTRACT_VALID configurations=1 paths=12")
        return
    end
    marker("AWGN_023B_QUEUE_START")
    ...
    length(targets) == (isempty(requested) ? length(A.CONFIGURATIONS) :
        ...
        "AWGN-023B received an unknown or duplicate experiment ID")
    marker("AWGN_023B_COMPUTE_START")
    foreach(run_configuration, targets)
    if isempty(requested)
        matrix_validator = joinpath(@__DIR__, "validate_matrix.py")
        run(`python3 $matrix_validator`)
    end
    marker("AWGN_023B_COMPUTE_COMPLETE")
    println("AWGN_023B_MATRIX_COMPLETE configurations=", length(targets),
