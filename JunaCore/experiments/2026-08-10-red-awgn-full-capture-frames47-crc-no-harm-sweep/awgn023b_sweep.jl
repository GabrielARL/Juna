...
const EXPECTED_PAYLOAD_BITS_PER_POINT = FRAMES * EXPECTED_PAYLOAD_BITS_PER_FRAME
...
const SNAPSHOT_INDICES = [AWGN022_SNAPSHOT_INDICES..., 3073, 3168, 3262, 3357, 3457, 3552, 3646, 3741, 3841, 3936, 4030, 4125, 4225, 4320, 4414]
const DATA_DIR = get(
...
# Repeat the approved four-frame placement in each complete four-second block.
# The first thirty-two positions remain the complete AWGN-022 workload schedule.
@eval B begin
    function _snapshot_positions(capture::ReplayCapture, packets::Integer,
        ...
        if count == 47
            positions = [1, 96, 190, 285, 385, 480, 574, 669, 769, 864, 958, 1053, 1153, 1248, 1342, 1437, 1537, 1632, 1726, 1821, 1921, 2016, 2110, 2205, 2305, 2400, 2494, 2589, 2689, 2784, 2878, 2973, 3073, 3168, 3262, 3357, 3457, 3552, 3646, 3741, 3841, 3936, 4030, 4125, 4225, 4320, 4414]
            last(positions) <= stop || throw(ArgumentError(
                "capture supports positions through $stop, needs 4414"))
            return positions
        end
...
function _require_file_hash(path, expected, description)
    isfile(path) || error("AWGN-023B missing $description: $path")
    islink(path) && error("AWGN-023B refuses symlinked $description: $path")
    actual = file_sha256(path)
    actual == expected || error(
        "AWGN-023B $description content differs: expected $expected, got $actual")
end
...
function require_source_contract()
    ...
    package_root == realpath(joinpath(RECEIVER_REPOSITORY, "JunaCore")) ||
        error("AWGN-023B loaded JunaCore from $package_root")
...
function require_contract(; verify_data::Bool=true)
    length(CONFIGURATIONS) == 1 || error("AWGN-023B configuration count differs")
    length(PATHS) == 12 || error("AWGN-023B path count differs")
    collect(SNR_DB) == collect(0:2:30) || error("AWGN-023B SNR grid differs")
    FRAMES == 47 && CAPTURE_SECONDS == 47.78125 && SEED == 4 || error(
        "AWGN-023B capture, frame, or seed contract differs")
    SNAPSHOT_INDICES == [AWGN022_SNAPSHOT_INDICES..., 3073, 3168, 3262, 3357, 3457, 3552, 3646, 3741, 3841, 3936, 4030, 4125, 4225, 4320, 4414] || error(
        "AWGN-023B nested replay positions differ")
    first(SNAPSHOT_INDICES, 32) == AWGN022_SNAPSHOT_INDICES || error(
        "AWGN-023B first thirty-two replay positions differ from AWGN-022")
    EXPECTED_PAYLOAD_BITS_PER_POINT == 75_952 || error(
        "AWGN-023B payload-bit count differs")
...
function _populate_capture_cache!(channel::Symbol, data_dir::AbstractString)
    ...
    isfile(file) || error("AWGN-023B missing measured capture: $file")
    islink(file) && error("AWGN-023B refuses symlinked measured capture: $file")
...
function require_snapshot_contract(capture, config)
    config == only(CONFIGURATIONS).config || error(
        "AWGN-023B snapshot contract received another configuration")
    size(capture.h) == (768, 4_588) || error(
        "AWGN-023B cropped tap geometry differs")
    length(capture.phase) == 917_600 || error(
        "AWGN-023B cropped phase geometry differs")
    frame_samples = 9_536
    _, stop = B._capture_position_limit(
        capture, frame_samples + F.PASSBAND_MODEM_PADDING_SAMPLES, A.MODEM_FS)
    stop == 4_488 || error("AWGN-023B capture position stop differs: $stop")
    ...
    maximum(ends) <= CAPTURE_SECONDS || error(
        "AWGN-023B replay support exceeds the complete measured capture")
    (
...
function _configure_protected_receiver(id::Symbol, capture, config)
    constructor = id === :profiled_cz ?
        ...
        JunaCore.JunaCrcConditionedJointCwzFrame.Modulation :
        error("AWGN-023B unknown protected receiver: $id")
    ...
    Modulations.isvalid(receiver, capture.fc, A.MODEM_FS) || error(
        "AWGN-023B public receiver $id is invalid after configuration")
    receiver
end
...
function evaluate(capture, channel_id, lane, config, snr_db;
    ...
    B._noise_override() === nothing || error(
        "AWGN-023B refuses a non-AWGN noise override")
    ...
    function decode(item, payload_bits, decoder_input, fc, fs)
        ...
        if id in (:ofdm_fec, :pfft, :lite)
            id === :lite && item.receiver.cz_crc_no_harm && error(
                "AWGN-023B Lite changed at decode boundary")
    ...
    function benchmark_sink(row)
        ...
        if id in (:profiled_cz, :cwz_joint)
            haskey(trace_bundles, id) || error(
                "AWGN-023B protected decode produced no no-harm trace")
    ...
    isempty(trace_bundles) || error(
        "AWGN-023B did not consume every protected trace")
    [(
...
function validate_path_rows(rows, frame_traces, protected_traces;
    ...
    for snr in expected_snrs, frame in 1:FRAMES
        ...
        length(paired) == length(ALGORITHMS) || error(
            "AWGN-023B paired workload receiver count differs")
        length(Set((row.workload_id, row.snapshot_index, row.payload_seed,
            ...
            "AWGN-023B receivers do not share one paired workload")
    end
    for receiver in RECEIVER_IDS, snr in expected_snrs
        ...
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-023B frame errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-023B frame successes do not reconcile")
    end
    for receiver in PROTECTED_RECEIVER_IDS, snr in expected_snrs
        ...
        frames = [row for row in protected_traces
                  if row.algorithm_id == receiver && row.snr_db == snr]
        sum(row.bit_errors for row in frames) == aggregate.bit_errors || error(
            "AWGN-023B protected errors do not reconcile")
        count(row -> row.success, frames) == aggregate.successful_frames || error(
            "AWGN-023B protected successes do not reconcile")
        all(row.selection_reason in ALLOWED_SELECTION_REASONS for row in frames) ||
            error("AWGN-023B protected selection reason differs")
    end
    all(row.optimized_variables ==
        (row.algorithm_id == "profiled_cz" ? "C+z" : "C+W+z")
        for row in protected_traces) || error(
            "AWGN-023B optimized-variable label differs")
...
function write_csv(destination::AbstractString, rows, expected_header)
    isempty(rows) && error("AWGN-023B refuses an empty CSV")
    keys(first(rows)) == Symbol.(expected_header) || error(
        "AWGN-023B CSV schema differs")
...
function run_path(channel::Symbol, lane::Integer, config;
    ...
    println(channel, " hydrophone ", lane,
            " AWGN-023B N=1024 CP=64 rate=0.25 pilots=5/5 dc=14 ",
