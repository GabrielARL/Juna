#!/usr/bin/env python3
"""Validate, combine, and render the approved AWGN-023B result."""
...
PATH_CONTRACT_BASENAME = "awgn023b_path_contract.txt"
...
class ResultContractError(RuntimeError):
    """One measured AWGN-023B artifact violated the approved contract."""


def require(condition, message):
    if not condition:
        raise ResultContractError("AWGN-023B result contract failed: " + message)
...
def _path_contract_record(results_dir, experiment_id, channel, hydrophone,
    ...
    expected = (
        ...
        ("runner_sha256", file_digest(os.path.join(HERE, "run_awgn023b.jl"))),
        ("sweep_sha256", file_digest(os.path.join(HERE, "awgn023b_sweep.jl"))),
        ("aggregate_rows", "80"),
...
def _validate_aggregate(rows, source, channel, hydrophone,
    ...
    for row in rows:
        ...
        require(float(row["capture_start_seconds"]) == 0.0
                and float(row["capture_stop_seconds"]) == 47.78125,
        ...
        require(int(row["payload_bits_per_frame"]) == 1616
                and int(row["payload_bits"]) == 47 * 1616,
...
def _validate_frame_traces(rows, source):
    ...
    for row in rows:
        ...
        require(float(row["replay_support_end_seconds"]) <= 47.78125 + 1e-9,
...
def _reconcile(aggregates, frame_rows, source, frame_count=47):
...
def load_renderer(receiver_ids):
    ...
    spec = importlib.util.spec_from_file_location("awgn023b_sweep_view", path)
...
def render_page(rows):
    ...
    title = "AWGN-023B: BER versus SNR"
    page = page.replace(
...
whose complete replay windows fit inside the measured capture. The first thirty-two
replay windows are unchanged from the thirty-two-second results; fifteen later replay
windows extend the measured observation to 47.78125 seconds. All five receivers use
...
def _family_source_hashes():
    names = (
        "awgn023b_contract_test.jl", "awgn023b_sweep.jl", "run_awgn023b.jl",
...
def build(experiment_dir, reference_results=AWGN022_REFERENCE_RESULTS,
    ...
    manifest = {
        ...
        "frame_window_selection": (
            ...
            "the first thirty-two are unchanged from the thirty-two-second result"),
    ...
    _atomic_text(os.path.join(results_dir, "results_view.html"),
                 render_page(rows))
    print("BUILT AWGN-023B: 960 rows, 45120 frame traces, 18048 protected "
