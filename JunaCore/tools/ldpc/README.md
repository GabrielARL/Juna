...
are the historically validated binaries with only that embedded string changed
to `src/randfile`; `LDPC.jl` launches them from this directory. This preserves
the parity matrices behind the recorded BER results while removing the original
...
change generated parity matrices, so it must be followed by the LDPC fingerprint
test and a deliberate regeneration of affected performance reports.
