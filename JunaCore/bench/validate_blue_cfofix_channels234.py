...
def blue1_snapshot(experiment):
    results = os.path.join(experiment, "results", "runs")
    snapshot = {}
    for lane in LANES:
        root = os.path.join(results, f"blue1_hydrophone{lane}")
        require(os.path.isdir(root), "missing Blue1 path " + root)
        for directory, _, names in os.walk(root):
            for name in sorted(names):
                path = os.path.join(directory, name)
                relative = os.path.relpath(path, experiment).replace(os.sep, "/")
...
def validate_manifest(experiment, expected_commit):
    ...
    require(rerun.get("path_count") == 9, "manifest rerun path count differs")
    require(rerun.get("blue1_preserved") is True,
            "manifest does not record preserved Blue1 evidence")
...
def finalize(experiment, snapshot_path, started_ms, expected_commit):
    ...
    require(blue1_snapshot(experiment) == before,
            "Blue1 evidence changed during a Blue2-Blue4-only rerun")
    ...
    print(
        f"{verdict} BLUE CFO FIX channels=blue2,blue3,blue4 paths=9 "
...
def main():
    ...
    if mode == "snapshot":
        ...
        with open(sys.argv[3], "w", encoding="utf-8", newline="\n") as handle:
            json.dump(blue1_snapshot(experiment), handle, indent=2, sort_keys=True)
            handle.write("\n")
        print("BLUE1 SNAPSHOT paths=3")
    elif mode == "finalize":
