...
combining vectors, and decodes again.  The physical response model
\(\mathbf C\) and the bit latents \(\mathbf z\) of the full JUNA objective are
not jointly optimized in JUNA-Lite.  The method is therefore a small
posterior-anchor refinement of ratio combining, not the full
\((\mathbf C,\mathbf W,\mathbf z)\) solver.  Joint use of pilot observations
...
The contribution is a compact mathematical description of this receiver using
...
values are retained.  The complete target set is
\begin{equation}
  \mathcal T^{(t)}
  =
  ...
  \label{eq:target-set}
\end{equation}
Define the target and target weight by
\begin{align}
  a_k^{(t)}
  &=
  \begin{cases}
...
carrier by a response measured on that carrier, so \(\widehat X_p=X_p\) to
numerical precision.  The score therefore separates candidates through \(E_c\),
the normalized syndrome weight, and \(\mu_c\); \(E_{\mathrm{pil},c}\) is
retained in the reported form because it would separate candidates for any
receiver that scored the combiner output \(\widetilde X_p\) before residual
pilot equalization.
...
A fair evaluation uses identical modulation, code, pilot pattern,
Partial-FFT branches, channel, signal-to-noise ratio, and packet budget.  The
primary comparison is pilot-only Partial-FFT plus BP against JUNA-Lite
...
of the observations and decoder information can provide a better
pre-decoder statistic than separate combining and decoding, while the seed,
confidence, ridge, comparison, and stopping rules preserve a controlled
fallback to the separate receiver.
