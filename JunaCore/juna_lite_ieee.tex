...
observations, and complex combiner weights form one pre-decoder soft symbol
per carrier.  A separate receiver estimates the combiner weights from pilots
...
combiner weights, and decodes again.  JUNA-Lite does not jointly optimize
the physical response model and the bit latents of the full JUNA objective.
The method is therefore a small posterior-anchor refinement of
partial-FFT combining, not the full JUNA solver.  Joint use of pilot observations
...
candidate comparison, and early stopping limit unreliable decoder feedback.
\end{abstract}

\begin{IEEEkeywords}
underwater acoustic communication, OFDM, Doppler, partial FFT, LDPC, belief
propagation, partial-FFT combining, iterative receiver.
\end{IEEEkeywords}
...
interval.  The frequency-domain operator is then no longer diagonal: its
...
The RLS-based Partial-FFT Combiner (RPC) estimates complex combiner weights
...
determines their weight.  The resulting receiver repeats partial-FFT combining
and decoding without introducing the full partial-FFT observation model of the general
JUNA formulation.
...
We assume an adequate cyclic prefix and consider only carrier coupling within
one OFDM symbol, so intersymbol interference (ISI) is outside this paper.  When
the channel does not vary over the block, cyclic-prefix removal leaves a
circulant operator.  The FFT
...
Motion time-scales the received packet.  After initial resampling a residual
Doppler factor \(a\) and a carrier frequency offset \(\theta\) remain, so carrier
...
residue is the ICI coefficient.
...
\(\Delta_{k,\ell}\), so for small \(a\) and \(\theta\) the ICI coefficients stay
near the diagonal and the operator is banded.  Read in the frequency domain, the
...
\(\sum_{m=1}^{M}g_m[n]=1\), the sum of the partial-FFT views equals the full FFT:
\begin{equation}
  \sum_{m=1}^{M}Y_{k,m}
  =
...
\section{Separate Partial-FFT Combining and Decoding}
...
confidence weights alone limit unreliable decoder feedback.  The complete target set
is
\begin{equation}
  \mathcal T^{(t)}
  =
...
not form or optimize the response \(\mathbf C\) in the partial-FFT observation
...
combiner weights.  This decoder feedback is useful when pilots alone do not specify
a stable combiner and when the decoder posterior information is reliable.  A
valid initial candidate produces no refinement, and unreliable decoder feedback
can reduce the value of the additional anchors.  Confidence selection, fixed
...
\(\mathbf C\) update in the partial-FFT observation model and the nonlinear
\(\mathbf z\) optimization
of the full JUNA solver.

We require identical modulation, code, pilot pattern,
partial-FFT views, channel, signal-to-noise ratio, and packet budget.  We
...
combining.  It starts from the same pilot-trained combiner used by the
