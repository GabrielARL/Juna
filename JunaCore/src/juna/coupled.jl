...
function _wcz_optimizer_config(m::Modulation)
  m.refinement_steps < 0 && return _COUPLED_PUBLIC_CONFIG
  base = _COUPLED_PUBLIC_CONFIG
  _CoupledOptimizerConfig(
    steps=m.refinement_steps,
    alpha_z=base.alpha_z,
    beta1=base.beta1,
    beta2=base.beta2,
    epsilon=base.epsilon,
    gradient_clip=base.gradient_clip,
    complex_value_clip=base.complex_value_clip,
    logit_clip=base.logit_clip,
  )
end
...
function _CoupledProblem(observations::AbstractMatrix{<:Number};
  ...
  nparts > 0 || throw(ArgumentError(
    "coupled observations need at least one partial-FFT view"))
...
# Deterministic feasible start: pilot-trained W, initial-candidate bit logits, and a
...
function _initial_coupled_state(m::Modulation,
  ...
  hasproperty(initial_candidate, :lpost_metric) ||
    throw(ArgumentError("coupled initialization requires initial-candidate posterior metrics"))
  metrics = initial_candidate.lpost_metric
  length(metrics) == problem.nbits ||
    throw(DimensionMismatch("initial-candidate metrics must have length $(problem.nbits)"))
...
function _coupled_objective(problem::_CoupledProblem,
  ...
  # Expected-likelihood term E|y-Cx|^2 = |y-Cmu|^2 + sum |C|^2 Var(x); the second
  # piece is exactly what the posterior-moment C solve puts on its Gram diagonal.
  # Including it aligns the z-objective with the EM-C objective so the profiled
  # z-gradient becomes the variable-projection total derivative.
  if expected_variance
    variances = _coupled_symbol_variances!(problem, state, scratch)
    @inbounds for k in problem.active
      response_group = _coupled_response_group(problem, k)
      for offset_pos in axes(problem.neighbor_idx, 1)
        q = problem.neighbor_idx[offset_pos, k]
        ...
        for branch in 1:nparts
          observation_sum +=
            var_q * abs2(state.C[branch, offset_pos, response_group, 1])
        end
      end
    end
  end
...
function _coupled_objective_and_gradient!(gradient::_CoupledGradient,
  ...
  # Posterior-variance term needs the same per-carrier variances the objective used.
  variances = expected_variance ?
    _coupled_symbol_variances!(problem, state, scratch) : nothing
  energy = expected_variance ? zeros(Float64, length(scratch.symbols)) : nothing
  ...
  if observation_scale > 0.0
    @inbounds for k in problem.active
      ...
      for offset_pos in axes(problem.neighbor_idx, 1)
        ...
        q == 0 && continue
        symbol = scratch.symbols[q]
        var_q = expected_variance ? variances[q] : 0.0
        for branch in 1:nparts
          ...
          if expected_variance
            # dL_var/dconj(C) = observation_scale * Var[q] * C ; and accumulate the
            # per-carrier channel energy A[q] = observation_scale * sum|C|^2.
            gradient.C[branch, offset_pos, response_group, 1] +=
              observation_scale * var_q * response
            energy[q] += observation_scale * abs2(response)
          end
        end
      end
    end
  end
  # dL_var/dconj(s_q) = -A[q] * s_q for each data symbol; the existing
  # symbol_gradient -> bit_gradient -> (dmu/dz) chain then finishes gradient.z.
  if expected_variance
    @inbounds for k in problem.data_idx
      problem.symbol_slots[k] == 0 && continue
      scratch.symbol_gradient[k] -= energy[k] * scratch.symbols[k]
    end
  end
...
function _coupled_hermitian_solve(gram::Matrix{ComplexF64},
                                  rhs::Vector{ComplexF64})
  try
    cholesky(Hermitian(gram)) \ rhs
  catch error
...
function _coupled_symbol_variances!(problem::_CoupledProblem,
                                    state::_CoupledState,
                                    scratch::_CoupledScratch)
  _coupled_symbols!(problem, state, scratch)
  variances = zeros(Float64, length(scratch.symbols))
  @inbounds for k in problem.data_idx
    problem.symbol_slots[k] == 0 && continue
    variances[k] = clamp(1.0 - abs2(scratch.symbols[k]), 0.0, 1.0)
  end
  variances
end

function _coupled_profile_C!(problem::_CoupledProblem,
                             state::_CoupledState;
                             weights::_CoupledWeights,
                             scratch::_CoupledScratch,
                             symbol_variances::AbstractVector{<:Real},
                             anchor,
                             trust::Real,
                             damping::Real)
  length(symbol_variances) == length(scratch.symbols) ||
    throw(DimensionMismatch("symbol variances must cover every carrier"))
  all(v -> isfinite(v) && v >= 0, symbol_variances) ||
    throw(ArgumentError("symbol variances must be finite and nonnegative"))
  isfinite(trust) && trust >= 0 ||
    throw(ArgumentError("C trust weight must be finite and nonnegative"))
  isfinite(damping) && 0 < damping <= 1 ||
    throw(ArgumentError("C damping must lie in (0,1]"))
  if anchor !== nothing
    size(anchor) == size(state.C) ||
      throw(DimensionMismatch("C anchor must match the response shape"))
  elseif trust > 0
    throw(ArgumentError("positive C trust requires an anchor"))
  end

  nparts = size(problem.observations, 1)
  noffsets = size(problem.neighbor_idx, 1)
  observation_scale = weights.observation / (nparts * length(problem.active))
  ridge_scale = weights.response_regularization / length(state.C)
  trust_scale = Float64(trust) / length(state.C)
  target_groups = problem.spec.response_parameterization === :per_carrier ?
                  [[k] for k in problem.active] : problem.bands
  design = zeros(ComplexF64, noffsets)
  gram = zeros(ComplexF64, noffsets, noffsets)
  rhs = zeros(ComplexF64, noffsets)

  @inbounds for (group_id, targets) in enumerate(target_groups), branch in 1:nparts
    fill!(gram, 0)
    fill!(rhs, 0)
    for k in targets
      for offset_pos in 1:noffsets
        q = problem.neighbor_idx[offset_pos, k]
        design[offset_pos] = q == 0 ? 0.0 + 0.0im : scratch.symbols[q]
      end
      observation = problem.observations[branch, k]
      gram .+= observation_scale .* (conj.(design) * transpose(design))
      for offset_pos in 1:noffsets
        q = problem.neighbor_idx[offset_pos, k]
        q == 0 && continue
        gram[offset_pos, offset_pos] +=
          observation_scale * symbol_variances[q]
      end
      rhs .+= observation_scale .* conj.(design) .* observation
    end
    for offset_pos in 1:noffsets
      gram[offset_pos, offset_pos] += ridge_scale + trust_scale
      if anchor !== nothing
        rhs[offset_pos] +=
          trust_scale * anchor[branch, offset_pos, group_id, 1]
      end
    end
    solution = _coupled_hermitian_solve(gram, rhs)
    previous = @view state.C[branch, :, group_id, 1]
    previous .= (1.0 - damping) .* previous .+ damping .* solution
  end
  state
end

# Exact minimizer of the C-dependent observation and ridge terms conditional
# on z. Response groups are independent in the deployed objective.
function _coupled_exact_C!(problem::_CoupledProblem,
                           state::_CoupledState;
  ...
  _coupled_profile_C!(
    problem, state;
    weights, scratch,
    symbol_variances=zeros(Float64, length(scratch.symbols)),
...
function _coupled_em_C!(problem::_CoupledProblem,
  ...
  _coupled_profile_C!(
    problem, state;
    weights, scratch, symbol_variances=variances,
    anchor, trust, damping)
end

# Exact minimizer of the pilot/tie, ridge, and inter-band smoothness terms
# conditional on z. A single system is used because smoothness couples bands.
function _coupled_exact_W!(problem::_CoupledProblem,
                           state::_CoupledState;
                           weights::_CoupledWeights = _COUPLED_RUNTIME_WEIGHTS,
                           scratch::_CoupledScratch = _CoupledScratch(problem),
                           pilot_positions=eachindex(problem.pilot_idx))
  _coupled_symbols!(problem, state, scratch)
  nparts = size(problem.observations, 1)
  ngroups = _coupled_combiner_groups(problem)
  nvariables = nparts * ngroups
  gram = zeros(ComplexF64, nvariables, nvariables)
  rhs = zeros(ComplexF64, nvariables)

  function add_target!(k::Int, target::ComplexF64, scale::Float64)
    group = _coupled_combiner_group(problem, k)
    indices = (1:nparts) .+ (group - 1) * nparts
    y = @view problem.observations[:, k]
    gram[indices, indices] .+= scale .* (y * y')
    rhs[indices] .+= scale .* y .* conj(target)
  end

  selected_pilots = collect(Int, pilot_positions)
  pilot_scale = isempty(selected_pilots) ? 0.0 :
                weights.pilot / length(selected_pilots)
  @inbounds for pilot_pos in selected_pilots
    k = problem.pilot_idx[pilot_pos]
    add_target!(k, problem.pilot_syms[pilot_pos], pilot_scale)
  end
  dynamic_count = count(k -> problem.symbol_slots[k] != 0, problem.data_idx)
  tie_scale = dynamic_count == 0 ? 0.0 : weights.tie / dynamic_count
  @inbounds for k in problem.data_idx
    problem.symbol_slots[k] == 0 && continue
    add_target!(k, scratch.symbols[k], tie_scale)
  end

  ridge_scale = weights.combiner_regularization / length(state.W)
  @inbounds for index in 1:nvariables
    gram[index, index] += ridge_scale
  end
  smoothness_count = nparts * max(ngroups - 1, 0)
  smoothness_scale = smoothness_count == 0 ? 0.0 :
                     weights.smoothness / smoothness_count
  @inbounds for group in 1:ngroups-1, branch in 1:nparts
    left = branch + (group - 1) * nparts
    right = left + nparts
    gram[left, left] += smoothness_scale
    gram[right, right] += smoothness_scale
    gram[left, right] -= smoothness_scale
    gram[right, left] -= smoothness_scale
  end
  state.W .= reshape(_coupled_hermitian_solve(gram, rhs), nparts, ngroups)
  state
end

function _coupled_posterior_W!(problem::_CoupledProblem,
                               state::_CoupledState;
                               weights::_CoupledWeights = _COUPLED_RUNTIME_WEIGHTS,
                               scratch::_CoupledScratch = _CoupledScratch(problem))
  _coupled_symbols!(problem, state, scratch)
  ...
  pilot_scale = isempty(problem.pilot_idx) ? 0.0 :
                weights.pilot / length(problem.pilot_idx)
  @inbounds for pilot_pos in eachindex(problem.pilot_idx)
    add_target!(
      problem.pilot_idx[pilot_pos],
      problem.pilot_syms[pilot_pos],
      pilot_scale)
  end
  ...
  @inbounds for k in problem.data_idx
    problem.symbol_slots[k] == 0 && continue
    reliability_sum += abs2(scratch.symbols[k])
  end
  @inbounds for k in problem.data_idx
    problem.symbol_slots[k] == 0 && continue
    reliability = abs2(scratch.symbols[k])
    reliability == 0.0 && continue
    add_target!(
      k, scratch.symbols[k],
      weights.tie * reliability / reliability_sum)
  end
  ...
  state.W .= reshape(
    _coupled_hermitian_solve(gram, rhs), nparts, ngroups)
  state
end
...
# The return value is always the best finite checkpoint seen,
...
function _coupled_wcz_solve(problem::_CoupledProblem,
  ...
  for iteration in 1:config.steps
    _coupled_exact_C!(problem, current; weights=weights, scratch=scratch)
    _coupled_exact_W!(problem, current; weights=weights, scratch=scratch)
    _coupled_objective_and_gradient!(
      gradient, problem, current; weights=weights, scratch=scratch)
    _adam_step_real!(
...
function _coupled_candidate(m::Modulation,
  ...
  initial = _initial_coupled_state(m, code, layout, problem, initial_candidate)
...
# initial candidate unless the shared validity/syndrome/score ordering accepts the result.
function _juna_wcz_candidate(m::Modulation,
  ...
  initial_candidate === nothing && (initial_candidate = _initial_candidate(m, code, layout, yparts))
  resolved_config = config === nothing ? _wcz_optimizer_config(m) : config
  candidate = _coupled_candidate(
    m, code, layout, yparts, initial_candidate; weights = weights, config = resolved_config,
  )
