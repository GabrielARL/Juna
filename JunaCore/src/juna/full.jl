...
function _juna_wz_candidate(m::Modulation, code::_Code, layout::_Layout,
  ...
  current = _juna_wz_adam_refine(
    m, code, layout, yparts, initial_candidate)
  _candidate_is_better(best, current) && (best = current)
  best
end

struct _WzGradientScratch
...
function _WzGradientScratch(m::Modulation, code::_Code)
  ntones = _ndata_tones(m, code.n)
  maxdeg = maximum((length(vars) for vars in code.check_vars); init=0)
  _WzGradientScratch(Vector{ComplexF64}(undef, ntones), Vector{Float64}(undef, code.n),
                   zeros(ComplexF64, ntones), zeros(Float64, code.n),
                   Vector{Float64}(undef, maxdeg), Vector{Float64}(undef, maxdeg))
end

function _juna_wz_adam_refine(m::Modulation, code::_Code, layout::_Layout,
                                 yparts, initial_candidate)
  W = _initial_pilot_W(m, yparts, layout)
  W0 = copy(W)
  z = Vector{Float64}(undef, length(initial_candidate.posterior_metric))
  @inbounds for i in eachindex(z)
    z[i] = clamp(
      -Float64(initial_candidate.posterior_metric[i]),
  ...
  best_candidate = _wz_state_candidate(m, code, layout, yparts, W, z)

  best_loss = _wz_loss_and_gradient!(m, code, layout, gW, gz, W, z, W0, z0, yparts,
                                 confidence, parity_sets, scratch)
  best_W = copy(W); best_z = copy(z)

  for iter in 1:_wz_refinement_steps(m)
    ...
    loss = _wz_loss_and_gradient!(m, code, layout, gW, gz, W, z, W0, z0, yparts,
                              confidence, parity_sets, scratch)
    if isfinite(loss) && loss < best_loss
      best_loss = loss; copyto!(best_W, W); copyto!(best_z, z)
    end
    candidate = _wz_state_candidate(m, code, layout, yparts, W, z)
    _candidate_is_better(best_candidate, candidate) && (best_candidate = candidate)
  end

  candidate = _wz_state_candidate(m, code, layout, yparts, best_W, best_z)
  _candidate_is_better(best_candidate, candidate) && (best_candidate = candidate)
  best_candidate
end

function _initial_pilot_W(m::Modulation, yparts, layout::_Layout)
  ...
  pilot_pos = zeros(Int, Int(m.fft_length))
...
function _wz_state_candidate(m::Modulation, code::_Code, layout::_Layout, yparts, W, z)
...
function _wz_symbol_grid!(m::Modulation, symbols, relaxed_bits, z)
  _bpc(m) == 2 || throw(ArgumentError(
    "JUNA-Wz implements QPSK only; got bits_per_data_carrier=$(m.bits_per_data_carrier)"))
  invsqrt2 = 1 / sqrt(2)
  fill!(relaxed_bits, 0.0)
  for t in eachindex(symbols)
    base = 2t - 1
    xr = tanh(0.5 * z[base])
    xi = base + 1 <= length(z) ? tanh(0.5 * z[base + 1]) : 0.0
    relaxed_bits[base] = xr
    base + 1 <= length(z) && (relaxed_bits[base + 1] = xi)
    symbols[t] = ComplexF64(xr, xi) * invsqrt2
  end
  symbols
end

function _wz_loss_and_gradient!(m::Modulation, code::_Code, layout::_Layout, gW, gz, W, z, W0, z0,
                            yparts, confidence, parity_sets, scratch)
  fill!(gW, 0.0 + 0.0im); fill!(gz, 0.0)
  fill!(scratch.symbol_gradient, 0.0 + 0.0im); fill!(scratch.bit_gradient, 0.0)
  _wz_symbol_grid!(m, scratch.symbols, scratch.relaxed_bits, z)

  ntones = min(length(scratch.symbols), length(layout.data_idx), length(confidence))
  loss = 0.0
  ...
  for t in 1:ntones
    base = 2t - 1
    xr = scratch.relaxed_bits[base]
    gz[base] += 2.0 * real(scratch.symbol_gradient[t]) * (0.5 * (1.0 - xr * xr) * invsqrt2)
    if base + 1 <= length(z)
      xi = scratch.relaxed_bits[base + 1]
      gz[base + 1] += 2.0 * imag(scratch.symbol_gradient[t]) * (0.5 * (1.0 - xi * xi) * invsqrt2)
    end
  end

  if _GRAD_LAMBDA_CODE > 0.0 && !isempty(parity_sets)
    lambda_scaled = _GRAD_LAMBDA_CODE / max(length(parity_sets), 1)
    loss += _parity_penalty_and_bit_gradient!(scratch.bit_gradient, scratch.relaxed_bits, parity_sets,
                                       lambda_scaled, scratch.prefix, scratch.clamped)
    for i in eachindex(z)
      dx = 0.5 * (1.0 - scratch.relaxed_bits[i] * scratch.relaxed_bits[i])
      gz[i] += scratch.bit_gradient[i] * dx
    end
  end
...
function _parity_penalty_and_bit_gradient!(bit_gradient::Vector{Float64},
                                    relaxed_bits::Vector{Float64},
  ...
  @inbounds for inds in parity_sets
    ...
    pf = 1.0
    for t in 1:d
      xi = clamp(relaxed_bits[inds[t]], -0.999, 0.999)
      clamped[t] = xi
      prefix[t] = pf
      pf *= xi
    end
    ...
    for t in d:-1:1
      bit_gradient[inds[t]] += scale * (prefix[t] * sf)
      sf *= clamped[t]
    end
  end
  lambda * pen
