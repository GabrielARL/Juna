...
# Start from the Partial-FFT initial candidate, then descend the reduced (W,z) objective
# with Adam, evaluating each step as a candidate and keeping the best decode.
function _juna_wz(m::Modulation, code::_Code, layout::_Layout, yparts, initial_candidate=nothing)
  _payload_from_metrics(m, code, _juna_wz_candidate(m, code, layout, yparts, initial_candidate).lpost_metric)
end

function _juna_wz_candidate(m::Modulation, code::_Code, layout::_Layout, yparts, initial_candidate=nothing)
  ...
  current = _juna_wz_gradient_solve(m, code, layout, yparts, initial_candidate)
...
function _juna_wz_gradient_solve(m::Modulation, code::_Code, layout::_Layout, yparts, initial_candidate)
  W = _initial_gradient_W(m, yparts, layout)
  W0 = copy(W)
  z = Vector{Float64}(undef, length(initial_candidate.lpost_metric))
  @inbounds for i in eachindex(z)
    z[i] = clamp(-Float64(initial_candidate.lpost_metric[i]), -_GRAD_CLIP_Z, _GRAD_CLIP_Z)
  end
  z0 = copy(z)
...
function _gradient_symbol_grid!(m::Modulation, S, xbit, z)
  _bpc(m) == 2 || throw(ArgumentError("JUNA-Wz implements QPSK only; got bpc=$(m.bpc)"))
  invsqrt2 = 1 / sqrt(2)
  fill!(xbit, 0.0)
