...
forms soft symbols with that estimate, and decodes afterward.  Each of these
three steps is a small problem, and when the pilots alone specify the
channel well, the separation costs little.  What separation costs in general
is mutual information, in both directions.  The parity constraints carry
information about the channel, because the channel acted on symbols that
must form a codeword.  The channel constraints carry information about the
bits, because the received signal ties the channel and the bits
together in \eqref{eq:general-forward}.  A receiver that fixes the channel estimate
before decoding uses neither piece of that information.
