...
function coupled_e2e_candidates(m, bits, snr_db::Real)
    ...
    initial_candidate = CoupledE2EJuna._initial_candidate(m, code, layout, yparts)
...
@testset verbose = true "Profiled C,z" begin
    ...
    @testset "internal coupled dispatch wins on the mismatched 0 dB channel" begin
        ...
        initial_candidate_errors = coupled_e2e_candidate_errors(
        ...
        @test (f.initial_candidate.syndrome, f.direct.syndrome,
               initial_candidate_errors, direct_errors) == (130, 99, 22, 17)
        @test direct_errors < initial_candidate_errors
    ...
    @testset "proxy-selected candidate has a bounded truth-error tradeoff" begin
        ...
        @test (f.initial_candidate.syndrome, f.dispatched.syndrome,
               initial_candidate_errors, selected_errors) == (16, 0, 1, 0)
        @test selected_errors <= initial_candidate_errors + 4
        @test selected_errors < length(bits) ÷ 2
        initial_candidate_syndrome = f.initial_candidate.syndrome
        selected_syndrome = f.dispatched.syndrome
        @info "Profiled C,z proxy/truth tradeoff" initial_candidate_syndrome selected_syndrome initial_candidate_errors selected_errors
    end
