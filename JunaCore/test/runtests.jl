...
const SUITES = [
    ...
    (key = "source-files", file = "source_file_check.jl",
     ...
     claim = "eleven selected JunaCore source files and three LDPC helper programs match the values stored in this repository, so an unreviewed edit is visible",
     origin = "written for Juna; the first stored values were recorded during migration",
     reader_title = "Source file check",
     reader_summary = "Eleven source files and three helper programs are compared with values stored by Juna. If one changes without review, this test fails.",
    ...
    (key = "packaging", file = "source_layout.jl",
     ...
     claim = "src/ carries the Lite, Profiled C,z, and Direct C,z closures, exposes the approved base, CRC, conditioned, and Direct C,z facades, retains Standard, and excludes unrelated receiver files and facades",
     origin = "adapted (inverted) from source_layout.jl",
     reader_title = "Files and receivers included in this package",
     reader_summary = "The required source files remain present; Standard, Partial-FFT, JUNA-Lite, Profiled C,z, and Direct C,z remain available; and unrelated receiver files and names remain absent.",
    ...
    (key = "interface", file = "public_modem_interface.jl",
     ...
     claim = "Standard, Partial-FFT, JUNA-Lite, and Profiled C,z cross the m-sequence, 3x3 fc/fs, waveform-length, metric-polarity, payload-rate, and error contracts; wrapper facades have receiver-specific suites",
     origin = "adapted from the source test and maintained by Juna",
     reader_title = "Four receiver modes send and recover test bits",
     reader_summary = "Standard, Partial-FFT, JUNA-Lite, and Profiled C,z send and recover data through the same functions at several carrier frequencies and sample rates, report the expected signal length and payload rate, and reject invalid inputs. Wrapper facades have receiver-specific suites.",
    ...
    (key = "pfft", file = "partial_fft_receiver.jl",
     ...
     claim = "the paper's OFDM+FEC and Partial FFT+FEC benchmark branches are public receiver modes pinned to their demodulate_methods columns, per-band combining survives the within-symbol channel one-tap cannot, lite never loses to its own initial candidate, and both baselines roundtrip BPSK",
     origin = "copied during migration and now maintained by Juna",
    ...
    (key = "direct-cz", file = "juna_direct_cz_frame.jl",
     tier = "receiver-specific", receivers = "receiver:direct_cz",
     title = "Direct C,z",
     claim = "the separate Direct C,z receiver applies simultaneous analytical descent to C and z on the pure J(C,z) objective, safeguards every proposal, derives W only for accepted decoder checkpoints, retains the best checkpoint, and preserves the CRC no-harm boundary",
     origin = "written for Juna under approved change JCM-237",
     reader_title = "Direct C,z",
     reader_summary = "The receiver updates the response and relaxed bits together, keeps fixed pilot bits unchanged, checks only accepted updates with the decoder, and returns a new result only when its CRC passes.",
     method = "It checks the public receiver settings, objective terms, analytical gradients, simultaneous update, clipping, backtracking, fixed bits, checkpoint selection, CRC selection, frame trace, and declared receiver chain.",
     reader_origin = "Written for this package."),
