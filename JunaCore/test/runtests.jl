...
const SUITES = [
    ...
    (key = "provenance", file = "provenance_contract.jl",
     title = "Executable provenance pins",
     claim = "the five migrated algorithm files and three LDPC helper binaries match their pinned sha256 digests from the source repository, so a silent fork is impossible",
     provenance = "authored for this package (fork detector vs sonique @ d49fff0)"),
