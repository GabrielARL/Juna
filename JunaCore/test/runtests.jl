...
const SUITES = [
    (key = "smoke", file = "phase1_smoke.jl",
     ...
     reader_summary = "The OFDM+FEC, partial fast Fourier transform (Partial-FFT), JUNA-Lite, and Profiled C,z receivers recover every bit from their own clean transmissions, and JUNA-Lite reports that it uses decoder results for a second equalization pass.",
    ...
    (key = "packaging", file = "source_layout.jl",
     ...
     claim = "src/ carries the Lite and Profiled C,z closures, exposes the approved base, CRC, and conditioned facades, retains JunaStandard as a compatibility alias, and excludes unrelated receiver files and facades",
     origin = "adapted (inverted) from source_layout.jl",
     reader_title = "Files and receivers included in this package",
     reader_summary = "The required source files remain present; OFDM+FEC, Partial-FFT, JUNA-Lite, and Profiled C,z remain available; and unrelated receiver files and names remain absent.",
