...
const SUITES = [
    (key = "smoke", file = "phase1_smoke.jl",
     ...
     claim = "the four receiver families in this smoke suite recover a clean-channel payload bit-exactly and Lite declares :posterior_anchor_ls",
     origin = "authored for this package (phase-1 migration gate)",
     reader_title = "Each receiver recovers its own clean test bits",
     reader_summary = "The Standard, partial fast Fourier transform (Partial-FFT), JUNA-Lite, and Profiled C,z receivers recover every bit from their own clean transmissions, and JUNA-Lite reports that it uses decoder results for a second equalization pass.",
    ...
    (key = "packaging", file = "source_layout.jl",
     ...
     claim = "src/ carries the Lite and Profiled C,z closures, exposes the approved base, CRC, and conditioned facades, retains Standard, and excludes unrelated receiver files and facades",
     origin = "adapted (inverted) from source_layout.jl",
     reader_title = "Files and receivers included in this package",
     reader_summary = "The required source files remain present; Standard, Partial-FFT, JUNA-Lite, and Profiled C,z remain available; and unrelated receiver files and names remain absent.",
    ...
    (key = "config", file = "receiver_configuration.jl",
     ...
     claim = "the supported runtime modes and Profiled C,z forms select distinct settings and init() resets to the benchmark geometry",
    ...
    (key = "interface", file = "public_modem_interface.jl",
     ...
     origin = "adapted from the source test and maintained by Juna",
     reader_title = "Every receiver sends and recovers test bits",
    ...
    (key = "contract", file = "interface_contract.jl",
     ...
     claim = "the descriptor catalog covers four receiver families; every receiver satisfies Modulations, executes its declared objective, and decodes a noiseless loopback payload-exactly",
     origin = "adapted from the source test and maintained by Juna",
     reader_title = "Standard, Partial FFT, JUNA-Lite, and Profiled C,z provide the same operations",
     reader_summary = "The receiver list contains Standard, Partial FFT, Lite, and Profiled C,z; each receiver provides the required operations, reports its stated refinement method, and recovers a clean payload bit for bit.",
    ...
    (key = "layout", file = "ofdm_layout.jl",
     ...
     reader_origin = "Copied during migration and now maintained by Juna."),
    (key = "acquisition-cfo", file = "acquisition_cfo.jl",
     tier = "mechanism", receivers = "stage:acquisition",
     title = "LFM carrier-offset and duration acquisition",
     claim = "the shared LFM acquisition separates signed carrier offset from frame-duration dilation and removes the carrier rotation before OFDM observations",
     origin = "authored for this package",
     reader_title = "Synchronization corrects carrier offset and duration",
     reader_summary = "The synchronization stage estimates positive and negative carrier offsets, removes their phase rotation, and still corrects a stretched frame.",
     method = "It applies fixed positive and negative carrier offsets and an independent duration change to deterministic synchronized waveforms.",
     reader_origin = "Written for this package."),
    (key = "replay-uacr", file = "replay_lane_contract.jl",
     tier = "mechanism", receivers = "stage:acquisition",
     title = "Measured-channel phase and delay replay",
     claim = "theta_hat applies phase only, phi_hat applies phase and delay with guarded cubic interpolation, and every replay stays inside measured channel and tracking support",
     origin = "written for Juna from the official UACR Python and MATLAB replay behavior",
     reader_title = "Measured-channel phase and delay replay",
     reader_summary = "The replay test distinguishes phase-only and phase-plus-delay tracking, preserves boundary samples with explicit guards, and rejects unsupported or non-monotone replay requests.",
     method = "It checks fixed cubic-interpolation values, snapshot indexing, adaptive guards, support bounds, boundary impulses, tracking records, and fixed-duration noise power.",
     reader_origin = "Written for this package from the official UACR replay behavior."),
    ...
    (key = "pfft", file = "partial_fft_receiver.jl",
     ...
     claim = "the paper's OFDM+FEC and Partial FFT+FEC benchmark branches are public receiver modes pinned to their demodulate_methods columns, sparse-QPSK phase states resolve under-sampled channel quadrants, per-band combining survives the within-symbol channel one-tap cannot, lite never loses to its own initial candidate, and both baselines roundtrip BPSK",
     origin = "copied during migration and now maintained by Juna",
     reader_title = "Standard and partial fast Fourier transform receivers",
     reader_summary = "The two receiver choices use distinct processing paths and recover a clean payload; sparse QPSK pilots recover channel-phase quadrants between distant pilots, and the partial transform handles the selected channel that changes within one symbol and defeats the Standard receiver.",
     method = "The test compares each public result with its direct receiver result, checks a deterministic sparse-pilot QPSK channel, exercises the selected changing channel, and checks six fixed noisy packets. Across those packets, JUNA-Lite must have no more aggregate bit errors than its Partial FFT starting result.",
     reader_origin = "Copied during migration and now maintained by Juna."),
    ...
    (key = "profiled-cz-crc", file = "juna_crc_profiled_cz_frame.jl",
     tier = "receiver-specific", receivers = "receiver:conditioned_joint_cwz",
     title = "Profiled C,z",
    ...
    (key = "wcz-solves", file = "juna_wcz_conditional_solves.jl",
     tier = "mechanism", receivers = "stage:profiled_cz",
     title = "Profiled C,z",
    ...
    (key = "profiled-cz-initialization", file = "juna_coupled_initialization.jl",
     tier = "mechanism", receivers = "stage:profiled_cz",
     title = "Profiled C,z",
     claim = "one Partial-FFT and FEC block maps into the constrained problem and deterministically initializes finite pilot-trained W, profiled C, and decoder-initialized z",
     origin = "copied from sonique research/JunaCoreTests @ d49fff0 and now maintained by Juna",
     reader_title = "Profiled C,z",
    ...
    (key = "profiled-cz-end-to-end", file = "juna_coupled_end_to_end.jl",
     tier = "mechanism", receivers = "stage:profiled_cz",
     title = "Profiled C,z",
     claim = "the internal coupled receiver decodes clean and impaired synthetic fixtures, bounds its candidate trade-off, and retains a valid initial candidate",
     origin = "copied from sonique research/JunaCoreTests @ d49fff0 and now maintained by Juna",
     reader_title = "Profiled C,z",
...
# Terminal output uses the plain-language layer (CL-24a). The technical
# title/claim/origin stay in this registry for suites.json and the
# explorer's Technical details.
function _print_map()
    for suite in SUITES
        println(rpad(suite.key, 18), suite.file)
        println("    ", suite.reader_title)
        println("    ", suite.reader_summary)
    end
end
...
function _run(selectors::Vector{String})
    ...
    @testset verbose = true "JunaCore (JUNA-Lite migration)" begin
        for suite in chosen
