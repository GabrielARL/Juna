...
const SUITES = [
    ...
    (key = "source-files", file = "source_file_check.jl",
     ...
     reader_summary = "Five algorithm files (Modulations.jl, LDPC.jl, common.jl, frame_wide_ldpc.jl, lite.jl) and three helper programs (make-ldpc, make-gen, print-pchk) are compared with values stored by Juna. If one changes without review, this test fails.",
    ...
    (key = "contract", file = "interface_contract.jl",
     ...
     origin = "adapted: scope narrowed to the three migrated facades",
     reader_title = "Standard, Partial FFT, and JUNA-Lite provide the same operations",
