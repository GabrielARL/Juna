...
const SUITES = [
    (key = "smoke", file = "phase1_smoke.jl",
     ...
     reader_summary = "The OFDM+FEC, partial fast Fourier transform (Partial-FFT), and JUNA-Lite receivers recover every bit from their own clean transmissions, and JUNA-Lite reports that it uses decoder results for a second equalization pass.",
    ...
    (key = "packaging", file = "source_layout.jl",
     ...
     claim = "src/ carries exactly the Lite closure (common, frame_wide_ldpc, lite), exposes JunaLite, JunaOFDMFEC, and JunaPartialFFT as the canonical facades, and retains JunaStandard only as a compatibility alias while every foreign variant file and facade is absent",
     origin = "adapted (inverted) from source_layout.jl",
     reader_title = "Files and receivers included in this package",
     reader_summary = "The required source files remain present; the OFDM+FEC, Partial-FFT, and JUNA-Lite receivers remain available, and the listed excluded receiver files and names remain absent.",
    ...
    (key = "contract", file = "interface_contract.jl",
     ...
     reader_summary = "The receiver list contains OFDM+FEC, Partial FFT, and Lite; each receiver provides the required operations, reports its stated refinement method, and recovers a clean payload bit for bit.",
