...
function coupled_receiver_fixture()
    ...
    initial_candidate = CoupledInitJuna._initial_candidate(m, code, layout, yparts)
...
@testset verbose = true "Profiled C,z" begin
    ...
    @testset "initial candidate initialization is deterministic finite and clamp-safe" begin
        f = coupled_receiver_fixture()
        problem = CoupledInitJuna._coupled_problem_from_receiver(
            f.m, f.code, f.layout, f.yparts,
        )
    ...
    @testset "initialization rejects non-finite observations and posterior metrics" begin
        ...
        observation_error = try
            CoupledInitJuna._initial_coupled_state(
                f.m, f.code, f.layout, bad_observations, f.initial_candidate,
            )
            nothing
        catch err
            err
        ...
        metrics[1] = NaN
        bad_initial_candidate = merge(f.initial_candidate, (; lpost_metric = metrics))
        metric_error = try
            CoupledInitJuna._initial_coupled_state(
