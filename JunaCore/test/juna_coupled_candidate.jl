...
function coupled_candidate_fixture()
    ...
    initial_candidate = CoupledCandidateJuna._initial_candidate(m, code, layout, yparts)
...
@testset verbose = true "Profiled C,z" begin
    ...
    @testset "coupled candidate selection cannot regress the initial candidate" begin
        ...
        direct = CoupledCandidateJuna._coupled_candidate(
            f.m, f.code, f.layout, f.yparts, f.initial_candidate; config = config,
        )
        ...
        @test !CoupledCandidateJuna._juna_better(f.initial_candidate, f.initial_candidate)
        @test !CoupledCandidateJuna._juna_better(selected, f.initial_candidate)
        @test CoupledCandidateJuna._payload_from_metrics(
    ...
    @testset "CoupledModulation declares and executes the coupled objective" begin
        ...
        alias = JunaCore.Juna.CoupledModulation()
