...
& Differentiate the coded-bit latents through the partial-FFT branch model
$\bm Y_k\approx\sum_q \bm c_{kq}s_q(\bm z)$.  Use
Eq.~\eqref{eq:universal_z_gradient}.
& Partial-FFT windows, branch count $M$, and branch-combiner weights should be
...
chosen.  Reliable soft symbols become code-constrained decision-directed pilots.
...
$\bm c_{kq}\in\C^M$ be the branch response from transmitted carrier $q$ to
observed carrier $k$.  With $\mathcal B_K(k)=\{q:\abs{q-k}\le K\}$,
\begin{equation}
  \bm r_k
  =
...
\begin{enumerate}
  ...
  branch count $M$, and ICI span $K_{\rm ICI}$.
  \item For each candidate, initialize channel/branch coefficients from pilots
  and optional sparse support selection.
  \item Alternate:
  \begin{enumerate}
    \item update channel/branch/interference variables by LS, grid, or proximal
    rule;
...
\begin{itemize}
  \item Do not gradient-descent integer quantities: ICI span, branch count,
