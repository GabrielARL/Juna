...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    {\itshape\normalsize\color{inkgray}
      RPC here denotes the Section II partial-FFT combining idea\par}
  \end{center}
\end{frame}
...
% =====================================================================
\begin{frame}{1. System Model (Partial-FFT Domain)}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.46\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.5em}
        \item Residual Doppler causes ICI: each received carrier depends on nearby transmitted symbols.
        \item Partial-FFT ($M$ branches) gives vector observation per carrier.
      \end{itemize}
    \end{column}
...
% =====================================================================
\begin{frame}{2. Where Doppler Enters}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.42em}
        \item A full FFT demodulates carrier $k$ over the whole OFDM symbol:
              \[
                y_k=\frac{1}{\sqrt{T}}\int_0^T r(t)e^{-j2\pi f_k t}\,dt .
              \]
        \item If the channel is static during the symbol, orthogonality leaves
              only $d_kH_k$ plus noise.
        \item Doppler makes the channel response $H_i(t)$ vary inside the
...
% =====================================================================
\begin{frame}{3. RPC as Section II Partial FFT}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        \item Divide the useful OFDM symbol into $M$ non-overlapping windows.
        \item FFT each short window:
              ...
              \]
        \item Approximate the channel in each short interval by its midpoint
              value.
      \end{itemize}
    \end{column}
    ...
    \begin{tikzpicture}
      \node[draw=ink,line width=0.6pt,fill=paper,
        ...
        {The weights act like a carrier-specific stepwise receiver window:\\
         the desired carrier adds more coherently and ICI is reduced.};
    \end{tikzpicture}
  \end{center}
...
% =====================================================================
\begin{frame}{3a. Where the Partial-FFT Equation Comes From}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.60\textwidth}
      Let $\am=\frac{m-1}{M}T$ and $\bmint=\frac{m}{M}T$.  After CP removal,
      \[
...
% =====================================================================
\begin{frame}{4. RPC vs JUNA (High-Level View)}

  \begin{center}
  \begin{tikzpicture}[x=1cm,y=1cm]
    % RPC label
    \node[anchor=west,font=\itshape\bfseries\color{ink}] at (-6.3,2.6)
      {RPC: Section II Partial-FFT Combiner};
...
% =====================================================================
\begin{frame}{5. RPC: What Is Used?}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.46\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        \item Section II defines the receiver front end: partial-FFT
              observations plus weighted combining.
        \item The combiner is chosen to compensate time variation and reduce
              ICI, but not to model coded symbols jointly.
  ...
  \vspace{0.15em}
  \begin{center}
    {\itshape\small Scalar partial-FFT combining maps the branch vector to one
    number before later decoding.}
  \end{center}
\end{frame}
...
% =====================================================================
\begin{frame}{5a. How This Helps With Doppler}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.25em}
        \item Doppler makes the channel vary inside one OFDM symbol, so the
              scalar FFT output mixes carriers.
        \item Partial FFT turns that continuous time variation into an
              $M$-branch vector:
              \[
	                H_i(t)\quad\leadsto\quad
	                \big[H_i(1),\ldots,H_i(M)\big].
	              \]
        \item The combiner uses these branch differences as degrees of freedom:
              align the desired carrier, reduce nearby ICI.
      \end{itemize}
  ...
  \vspace{0.2em}
  \begin{center}
    {\itshape\footnotesize RPC does not estimate every coded symbol jointly; it first
    builds a Doppler-compensated scalar observation for each carrier.}
  \end{center}
...
% =====================================================================
\begin{frame}{5b. Diagram: Branch Projection View}
...
% =====================================================================
\begin{frame}{6. Normal Equations and RLS}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        \item For nearby pilots $\mathcal P(k)$, choose $\vW_k$ so that
              $\vW_k^{\herm}\vY_p$ matches the known pilot $X_p$.
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw=ink,line width=0.6pt,fill=paper,
            minimum width=13.8cm,minimum height=1.0cm,
            align=center,inner sep=6pt]
        {RLS is the sequential form: as pilot rows arrive, update
         $\bm P_t\approx\bm R_t^{-1}$,\\ then update
...
% =====================================================================
%  Slide 7: Lifting Partial-FFT to a Branch-Domain Model
% =====================================================================
\begin{frame}{7. From RPC's Combiner to JUNA's Branch Model}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \textbf{\color{rpcred}RPC view.} Squash $\vY_k$ to one number, then decode:
      \[
        \hX_k \;=\; \vW_k^{\herm}\,\vY_k .
      \]
  ...
  \begin{center}
    {\itshape\small RPC compresses $\vY_k$ before the code is used.\;
...
% =====================================================================
\begin{frame}{8. Where the Off-Diagonal $\vc_{k,q}$ Come From}

  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.44\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.22em}
        \item Static: branches at $k$ depend on $X_k$ only $\Rightarrow$ only $\vc_{k,k}$ nonzero.
        \item Doppler makes $H_i(t)$ \emph{vary inside} every short window; each neighbor $q$ leaks into branch $m$ with its own weight.
        \item Stack across $M$ branches $\Rightarrow$ vector $\vc_{k,q}\in\Cx^{M}$.
        \item Leakage decays with $|k-q|$; keep $|k-q|\le K_{\rm ICI}$ $\Rightarrow$ \textbf{banded}.
        \item These are the ICI terms RPC tries to cancel; JUNA \emph{models} them.
      \end{itemize}
    \end{column}
...
% =====================================================================
\begin{frame}{9. The Banded Mixing Matrix: $\vY=\vC\,\vs+\vn$}
...
% =====================================================================
\begin{frame}{10. Why Pilots Help}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      ...
      \vspace{0.2em}
      Pilots also let RPC train $\vW_p^{\herm}\vY_p\!\approx\!P_p$, giving JUNA a meaningful warm start.
    \end{column}
...
\framesubtitle{one cost over $(\vC,\vW,\vz)$ --- five jobs in one place}
...
% =====================================================================
\begin{frame}{12. What Each Term Buys You}
  ...
  \begin{tabular}{|>{\columncolor{labelbg}\bfseries\footnotesize}p{2.5cm}
    ...
    \textcolor{boxblue}{(B) pilot anchor}
      ...
      & The classical RPC training cost. Pins $\vW_p$ to known targets, and (via the bilinear coupling) helps identify $\vc_{k,p}$. \\
    \hline
    ...
    \textcolor{ink}{(D) ridge}
      ...
      & Stabilises ill-conditioned inverses (sparse pilots, banded but high-dimensional $\vC$). \\
    \hline
    ...
    \textcolor{junagreen}{(F) parity}
      ...
      & Lets the LDPC code shape the front end --- not only the post-detector. Where pilots run out, parity provides anchoring. \\
    \hline
  \end{tabular}
  \end{center}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw=ink,fill=paper,line width=0.6pt,minimum width=14cm,minimum height=0.95cm,align=center,inner sep=5pt]
        {Drop (A), (C), (E), (F): the $\vW_k$ update collapses to RPC's pilot-trained ridge/RLS system.\\
         JUNA $=$ RPC \emph{plus} branch model, soft-symbol coupling, smoothness, and code.};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{13. Alternating Solver}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.82,every node/.style={transform shape}]
    ...
    \node[flowbox,minimum width=1.8cm] (init) at (-3.8,0) {RPC\\init};
...
% =====================================================================
\begin{frame}{14. Key Difference at a Glance}
  ...
  \begin{tabular}{|>{\columncolor{labelbg}\bfseries}p{2.7cm}
    ...
    \multicolumn{1}{|c|}{\bfseries Aspect}
      & \multicolumn{1}{c|}{\bfseries\color{rpcred} RPC}
    ...
    \hline
    Code use
      & External to RPC; after scalarization
    ...
    Effect
      ...
      & Exploits local ICI and code constraints jointly \\
    \hline
  \end{tabular}
  \end{center}
...
% =====================================================================
\begin{frame}{15. Why Does This Matter?}
\framesubtitle{(Information-Theoretic View)}

  \begin{itemize}\setlength{\itemsep}{0.25em}
    \item RPC forms $\hX = g(\vY)$ using Section II partial-FFT combining, then decodes.
...
% =====================================================================
\begin{frame}{16. Information Flow Illustration}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.92,every node/.style={transform shape}]
    ...
    \node[font=\itshape\bfseries\color{rpcred},anchor=north]
      at ($(rpc.north)+(0,-0.2)$) {RPC (Section II Scalarization Before Decoding)};
...
\begin{frame}{17. Takeaway}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35em}
    \item RPC is the Section II idea: split one OFDM symbol into partial-FFT
          branches, then combine them with $\vW_k$.
    \item It helps Doppler because each short interval is closer to
          time-invariant, and weighted combining reduces ICI before scalar detection.
    \item JUNA asks what local branch-domain model explains the observations
          using nearby coded symbols.
    \item Scalar combining may hide branch structure; JUNA keeps it and couples
          it with LDPC constraints, allowing reliable symbols to act as soft pilots.
  \end{itemize}
