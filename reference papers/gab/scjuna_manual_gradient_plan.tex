...
The hand-derived gradient is short because all optimized bit variables are real.  The complex channel and branch quantities remain fixed during the $\bm z$ update, so the gradient only requires real derivatives through the relaxed QPSK map and the LDPC parity products.  The recommended implementation is
\begin{equation}
\boxed{
