...
\begin{abstract}
This document specifies how to implement the SC-JUNA bit-latent update without relying on automatic differentiation.  Closed-form least-squares updates serve the partial-FFT observation model and the combiner, and a hand-derived real gradient serves the latent vector.  Automatic differentiation may be used only as a test-time gradient checker.  The planned SC-JUNA code should use the formulas in this document.
\end{abstract}
...
Use the hand-derived gradient in Section~\ref{sec:z-gradient}.  Do not use complex automatic differentiation (AD) inside the production solver.
...
\paragraph{Reason.}
The $\bm z$ variables are real, while the observation residuals are complex.  Although AD removes the need to derive gradients by hand, several Julia packages handle complex-valued intermediate computations differently.  A manual real gradient avoids package-specific complex-AD conventions and makes the implementation deterministic.
...
Partial-FFT windows & nested rectangular only & Guarantees partition-of-unity and allows adjacent partial-FFT views to be merged. \\
...
\bottomrule
\endfoot
\texttt{Ypartial} & \texttt{N x M} & $Y_{k,m}$, partial-FFT view for carrier $k$ and view $m$. \\
...
The formulas in this section define $z_i$ as the bit latent for the planned
SC-JUNA update.  The current frame-wide JUNA-Wz path calls $z_i$ a logit and
maps it with $\tanh(z_i/2)$ in
\texttt{src/juna/frame\_wide\_ldpc.jl}.  Its chain rule therefore carries the
factor $1/2$.  These are different code paths; the formulas below apply only
to the bit-latent update specified here.
...
\midrule
$\lambda_d$ & $1.0$ & partial-FFT observation consistency \\
...
\item Implement forward pass: $\bm z\mapsto\bm\xi\mapsto\mathbf s$, combiner outputs, and partial-FFT observation residuals.
...
\item $\mathbf C$ update reduces the partial-FFT observation residual in a deterministic toy case.
...
The hand-derived gradient is short because all optimized bit variables are real.  The complex quantities $\mathbf C$ and $\mathbf W$ remain fixed during the $\bm z$ update, so the gradient only requires real derivatives through the relaxed QPSK map and the LDPC parity products.  The recommended implementation is
\begin{equation}
\boxed{
...
\end{equation}
While AD is useful for experimentation, it should not be the production dependency for the SC-JUNA solver.
