...
\[
  f_\ell^{\rx}=(1+a)\ell+\nu,
\]
where $a$ is Doppler scaling and $\nu$ is a carrier frequency offset.
...
\begin{tikzpicture}[x=0.78cm,y=0.78cm,>=Latex]
  ...
  \node[bluecard,text width=5.5cm,font=\scriptsize] at (0,-2.35)
    {This red leftover arrow is the ICI term from carrier $\ell$ in bin $k$.};
\end{tikzpicture}
\end{center}
\end{column}
\end{columns}
...
\[
  y_k=c_{k,k}d_k+n_k.
\]
With Doppler, the same FFT bin receives ICI terms from neighbouring carriers:
\[
  y_k=c_{k,k}d_k+
...
\begin{tikzpicture}[x=0.42cm,y=0.42cm]
  ...
  \node[font=\scriptsize\color{accentred},align=center] at (11.7,4.8) {ICI};
