...
\begin{tikzpicture}[x=1.0cm,y=1.0cm,>=Latex,
  ...
  \node[roadcard] (s4) at (3.3,-1.0) {%
    ...
    \scriptsize partial-FFT views formed from short windows; weights line up the wanted carrier};
...
\begin{center}\footnotesize
  ...
  \textcolor{accentred}{\textbf{red}}=ICI coefficients,\quad
...
\stepbadgehdr{3}{the failed cancellation loop becomes an ICI coefficient}
\small
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    \textbf{No Doppler.}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.49\textwidth}
    ...
    \begin{tikzpicture}[x=0.82cm,y=0.82cm]
      ...
      \node[font=\scriptsize\color{accentred}] at (0,-1.55) {non-zero residue = ICI coefficient};
    \end{tikzpicture}
    \end{center}
  \end{column}
\end{columns}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.45\textwidth}
    ...
    \[
      ...
      + n_k.
    \]
    The off-diagonal terms are \textbf{ICI}---pieces of symbols on other
    carriers in the same OFDM symbol inside bin $k$.
    \vspace{0.20cm}
    \begin{tikzpicture}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.47\textwidth}
    For one OFDM symbol, every transmitted carrier leaves some coefficient in
    every FFT bin:
    \[
      y_k=\sum_\ell c_{k,\ell}d_\ell+n_k.
    \]
    Stack its bins and carrier symbols:
    \[
  ...
  \begin{column}{0.51\textwidth}
    ...
    \begin{tikzpicture}[x=0.43cm,y=0.43cm,>=Latex]
      ...
      \node[font=\tiny\color{accentred},align=center] at (4.0,2.80) {$\pm1,\pm2$\\ICI coefficients};
...
% =====================================================================
% 3f-i. Carrier and OFDM-symbol indices
% =====================================================================
\begin{frame}{3f-i. ICI and ISI}
\stepbadgehdr{3}{use carrier and OFDM-symbol indices}
\scriptsize
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    We now keep the OFDM-symbol index:
    \[
      \mathbf y_s
      =\mathbf C\mathbf d_s
      +\mathbf J_-\mathbf d_{s-1}
      +\mathbf J_+\mathbf d_{s+1}
      +\mathbf n_s,
    \]
    where $\mathbf d_r$ collects carrier symbols from OFDM symbol $r$,
    $\mathbf C$ acts within $s$, $\mathbf J_\pm$ act across OFDM symbols, and
    $\mathbf n_s$ is noise.

    \vspace{0.05cm}
    The one-tap model requires both conditions:
    \begin{center}
    \begin{tikzpicture}[x=1cm,y=0.58cm]
      \node[greencard,text width=5.1cm,font=\scriptsize,inner sep=3pt] at (0,0.40)
        {prefix covers the delay spread};
      \node[greencard,text width=5.1cm,font=\scriptsize,inner sep=3pt] at (0,-0.50)
  ...
  \begin{column}{0.49\textwidth}
    \begin{center}
    \begin{tikzpicture}[x=0.76cm,y=0.62cm,>=Latex]
      \node[notecard,text width=1.55cm,font=\scriptsize,inner sep=3pt] (prev) at (-3.2,1.30)
        {preceding\\$\mathbf d_{s-1}$};
      \node[greencard,text width=1.55cm,font=\scriptsize,inner sep=3pt] (curr) at (-3.2,0)
        {current\\$\mathbf d_s$};
      \node[notecard,text width=1.55cm,font=\scriptsize,inner sep=3pt] (next) at (-3.2,-1.30)
        {following\\$\mathbf d_{s+1}$};
      \node[bluecard,text width=1.50cm,font=\scriptsize,inner sep=3pt] (out) at (2.6,0)
        {output\\$\mathbf y_s$};
      \draw[flowarr,color=accentred] (prev) -- node[above,font=\tiny] {$\mathbf J_-$} (out);
      \draw[flowarr,color=accentgreen] (curr) -- node[above,font=\tiny] {$\mathbf C$} (out);
      \draw[flowarr,color=accentred] (next) -- node[below,font=\tiny] {$\mathbf J_+$} (out);
    \end{tikzpicture}
    \end{center}

    \begin{center}
    \begin{tikzpicture}[y=0.58cm]
      \node[redcard,text width=5.25cm,font=\scriptsize,inner sep=3pt] at (0,0.35)
        {same $s$, different carrier: ICI coefficient};
      \node[redcard,text width=5.25cm,font=\scriptsize,inner sep=3pt] at (0,-0.55)
        {different OFDM-symbol index: intersymbol interference (ISI)};
    \end{tikzpicture}
    \end{center}

    For a causal channel with an aligned FFT window, $\mathbf J_+=\mathbf 0$.
  \end{column}
\end{columns}
\bottomnote{\scriptsize Carrier indices separate ICI; OFDM-symbol indices separate ISI.}
\end{frame}

% =====================================================================
% 3f-ii. Prefix shortfall and conditional scaling
% =====================================================================
\begin{frame}{3f-ii. Insufficient cyclic prefix}
\stepbadgehdr{3}{state what stays fixed before changing $N$}
\scriptsize
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    A tap at delay $m>L_{\rm cp}$ carries the preceding OFDM symbol into the
    first $m-L_{\rm cp}$ useful samples.  Across all taps,
    \[
      \begin{aligned}
        D&=\max(0,L_h-1-L_{\rm cp}),\\[-0.1em]
        \text{contaminated fraction}&=\frac{D}{N},
      \end{aligned}
    \]
    where $L_h$ is the number of channel taps, $L_{\rm cp}$ is the prefix
    length, $D$ counts contaminated samples, and $N$ is the number of samples
    in the useful interval.

    \begin{center}
    \begin{tikzpicture}[x=0.65cm,y=0.55cm]
      \draw[fill=blubg,draw=accentblue] (-3.1,0.70) rectangle (3.1,1.25);
      \node[font=\scriptsize] at (0,0.98) {channel memory $L_h-1$};
      \draw[fill=grnbg,draw=accentgreen] (-3.1,-0.20) rectangle (1.25,0.30);
      \node[font=\scriptsize] at (-0.95,0.05) {prefix $L_{\rm cp}$};
      \draw[fill=redbg,draw=accentred] (1.25,0.70) rectangle (3.1,1.25);
      \node[font=\scriptsize\color{accentred}] at (2.18,0.98) {$D$};
      \draw[brace] (1.25,1.38) -- (3.1,1.38);
    \end{tikzpicture}
    \end{center}

    \begin{center}
    \begin{tikzpicture}
      \node[redcard,text width=5.1cm,font=\scriptsize,inner sep=3pt]
        {short CP: $\mathbf C$ generally non-diagonal, $\mathbf J_-$ nonzero};
    \end{tikzpicture}
    \end{center}
  \end{column}
  \begin{column}{0.49\textwidth}
    ...
    \begin{tikzpicture}[x=1cm,y=0.72cm]
      \node[bluecard,text width=5.25cm,font=\scriptsize,inner sep=3pt] at (0,1.50) {%
        fixed $L_h$ and $L_{\rm cp}$ in samples:\\[0.15em]
        $\displaystyle \frac{D}{N}\propto N^{-1}$};
      \node[bluecard,text width=5.25cm,font=\scriptsize,inner sep=3pt] at (0,-0.05) {%
        fixed residual factor $a$, carrier frequency $f_\ell$, and bandwidth $B$:\\[0.15em]
        $\displaystyle \frac{a f_\ell}{B/N}=\frac{a f_\ell N}{B}\propto N$\\[-0.05em]
        \tiny offset in carrier spacings grows; ICI magnitude need not be monotonic};
      \node[notecard,text width=5.25cm,font=\scriptsize,inner sep=3pt] at (0,-2.00) {%
        If the prefix, bandwidth, sampling rate, or residual offset changes
        with $N$, both scalings must be recalculated.};
    \end{tikzpicture}
    \end{center}
  \end{column}
\end{columns}
\bottomnote{\scriptsize These conditional model scalings do not identify the cause of a measured search result.}
\end{frame}
...
% =====================================================================
\begin{frame}{3i. One partial-FFT view: the same sum splits by tone}
\stepbadgehdr{3}{linearity means wanted and ICI pass through the view separately}
\small
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.47\textwidth}
    Suppose the received samples contain the wanted tone and one interfering tone:
    \[
      r[n]=d_k a_k[n]+d_\ell a_\ell[n]+\cdots
    \]
    where $a_\ell[n]$ means ``tone $\ell$ after channel and Doppler.''

    Partial-FFT view $m$ is the sum over its segment $\mathcal S_m$:
    \[
      z_{m,k}
  ...
  \begin{column}{0.51\textwidth}
    ...
    \begin{tikzpicture}[x=0.86cm,y=0.62cm,>=Latex]
      ...
      \node[redcard,text width=2.00cm,font=\scriptsize,align=center] (leak) at (1.65,0)
        {ICI\\coefficient\\$h_{m;k,\ell}$};
      \node[notecard,text width=5.45cm,font=\scriptsize,align=center] at (0,-1.60) {%
        One partial-FFT view is already a small mixture:
...
% =====================================================================
\begin{frame}{3j. Stack four views: now the ICI has a shape}
\stepbadgehdr{3}{the vector is just four equations, one for each view}
\footnotesize
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.52\textwidth}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    Use the partial-FFT observation vector from slide 3j:
    \[
    ...
    \vspace{-0.15cm}
    Substitute the desired and ICI coefficient patterns:
    \[
      y_k^{\rm full}
      =
      ...
      d_k
      +
      \underbrace{\mathbf w_{\rm full}^{\mathsf H}\mathbf h_{k,\ell}}_{\text{ICI coefficient}}
      d_\ell+\cdots .
    \]
  \end{column}
  \begin{column}{0.50\textwidth}
    ...
    \begin{tikzpicture}[x=0.78cm,y=0.70cm,>=Latex]
      ...
      \node[redcard,text width=5.35cm,font=\scriptsize,align=left] at (-0.10,-1.55) {%
        ...
        If the ICI coefficient pattern does not cancel under equal weights, it remains.\\[0.15em]
...
% =====================================================================
% 4b. Partial FFT keeps short-window branches
% =====================================================================
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.46\textwidth}
    ...
    \vspace{0.20em}
    Stack them into a partial-FFT observation vector
    \[
      \vY_k = [z_{0,k},\, z_{1,k},\, z_{2,k},\, z_{3,k}]\T .
    \]

    \vspace{0.10em}
    {\scriptsize The partial-FFT observation vector keeps how the symbol \emph{changed} across $T$ — exactly the information the full FFT averaged away.}
	  \end{column}
  \begin{column}{0.52\textwidth}
    \begin{center}
    \begin{tikzpicture}[x=0.88cm,y=0.88cm,>=Latex]
      ...
      \node[idea,minimum width=5.0cm,font=\scriptsize,inner sep=4pt] (vec) at (0,-1.60)
        {partial-FFT observation vector $\vY_k$ — keeps how the symbol changed};
...
\stagefourhdr{2}{check: an equal average reconstructs the full-FFT ICI coefficient}
\small
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    For the ICI coefficient from $\ell=5$ into $k=4$ (the carrier we are tracking from earlier slides), the four short-window coefficients are
    \[
      \begin{aligned}
      ...
      \end{aligned}
    \]
    Each $c^{(m)}$ is the contribution to the ICI coefficient \emph{measured within window $m$}.
  \end{column}
  \begin{column}{0.48\textwidth}
    \begin{tikzpicture}
      \node[notecard,text width=0.96\linewidth,inner sep=6pt,
        ...
        \[
          ...
          \;=\; 0.128 + 0.117\jj,
        \]
        which is exactly the full-FFT ICI coefficient $c_{4,5}$ from slide 4a.
      };
    \end{tikzpicture}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    Each partial-FFT view contains a desired piece $c_{m,k,k}d_k$ plus ICI.
...
% =====================================================================
% 4d. Same weights make ICI less coherent
% =====================================================================
\begin{frame}{4d. The same weights make the ICI less coherent}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    ...
    \]
    For $\ell=k$, weights were chosen so terms add. For $\ell\ne k$, the ICI terms typically have \emph{different phases in each view}, so after the same alignment they partly cancel.

    \vspace{0.2cm}
    \begin{tikzpicture}
      \node[bluecard,text width=0.90\linewidth,inner sep=3pt,font=\scriptsize,align=left] {%
        \textbf{Same weights on the $\ell=5$ ICI coefficient.}\\
        After desired alignment, the ICI coefficient becomes roughly\\
  ...
  \begin{column}{0.50\textwidth}
    \begin{center}
    \begin{tikzpicture}[x=0.78cm,y=0.78cm,>=Latex]
      \node[font=\small\bfseries\color{accentred}] at (-2.5,2.25) {full-FFT ICI coefficient};
      ...
      \node[font=\small\bfseries\color{accentgreen}] at (2.5,2.25) {partial-FFT ICI coefficient};
...
\stagefourhdr{5}{the diagonal stays loud, many ICI coefficients get small}
\small
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    ...
    \[
      ...
      +\tilde n_k.
    \]
    Example: $|c_{4,5}/c_{4,4}|\approx 0.189$. Several visible ICI coefficients.
  \end{column}
  \begin{column}{0.49\textwidth}
    ...
    \[
      ...
      +\tilde n_k.
    \]
    Same example: $|\widetilde c_{4,5}/\widetilde c_{4,4}|\approx 0.007$. Many ICI coefficients shrink.
  \end{column}
\end{columns}
...
\bottomnote{Same diagonal height; the off-diagonal ICI coefficients are much smaller after combining.}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    Partial FFT gives the receiver a partial-FFT observation vector:
    \[
...
\begin{tikzpicture}[x=1.0cm,y=0.82cm,>=Latex]
  ...
  \node[method, minimum width=2.7cm,minimum height=0.95cm,font=\scriptsize\bfseries] (d) at (5.5,0.95)
    {4. partial FFT\\keeps partial-FFT views};
...
\begin{tikzpicture}
  \node[draw=accentgreen,fill=grnbg,line width=0.9pt,rounded corners=4pt,
    ...
    Motion turns each carrier's closed loop into an unfinished spiral; partial FFT keeps multiple partial-FFT views formed from short windows so the receiver can rotate the desired arrow back into alignment while the ICI arrows scramble.
  };
\end{tikzpicture}
\end{center}
