...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    ...
    \begin{enumerate}\setlength{\itemsep}{0.35em}
      ...
      \item Why does Doppler make another carrier cause ICI in my bin?
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{tikzpicture}
      \node[bluecard,text width=6.1cm] {%
        If you understand the ICI from carrier $5$ into bin $4$, the matrix story is just many copies of the same event.};
    \end{tikzpicture}
  \end{column}
\end{columns}
...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex]
  ...
  \node[redcard,text width=2.2cm] (c) at (6.0,1.2) {$c_{k,\ell}$\\ICI coefficient};
  \draw[flowarr] (d) -- (tone);
  \draw[flowarr] (tone) -- (x);
  \draw[flowarr] (x) -- (bin);
  \draw[flowarr] (bin) -- (c);
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.50\textwidth}
    ...
    \[
      ...
      \nu=\text{carrier frequency offset}.
    \]
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.50\textwidth}
    ...
    \]
    That scalar is already the final average. It no longer remembers how the desired arrow and the arrow for the ICI coefficient moved during the symbol.
...
\begin{columns}[T,onlytextwidth]
  ...
  \end{column}
\end{columns}
\takeaway{Partial FFT changes the observation from one scalar into a partial-FFT observation vector.}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.48\textwidth}
    ...
    \begin{tikzpicture}[x=0.82cm,y=0.82cm,>=Latex]
      ...
      \node[font=\scriptsize\bfseries\color{accentred}] at (2.2,2.0) {ICI after weights};
  ...
  \end{column}
\end{columns}
\takeaway{Partial FFT works because desired and ICI coefficients evolve differently across short windows.}
\end{frame}

% =====================================================================
\begin{frame}{A tiny before/after number}
\storybadge{4}{same example, smaller ICI coefficient}
\small
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.45\textwidth}
    ...
    \[
      ...
      \qquad\text{(full FFT)}.
    \]
    With $M=4$ partial-FFT views and weights chosen for bin $4$:
    \[
      \left|\frac{\widetilde c_{4,5}}{\widetilde c_{4,4}}\right|
      \approx 0.0066.
    \]
    That is about $29$ dB smaller selected ICI coefficient.
  \end{column}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.48\textwidth}
    Partial FFT creates the partial-FFT observation vector $\mathbf z_k$. The receiver still needs good weights $\mathbf w_k$.
    \vspace{0.20cm}
  ...
  \begin{column}{0.50\textwidth}
    ...
    \begin{tikzpicture}[x=0.88cm,y=0.88cm,>=Latex]
      ...
      \node[greencard,text width=2.2cm] (o1) at (3.0,1.0) {combine\\the views};
      \draw[flowarr] (p1)--(rpc);
      \draw[flowarr] (rpc)--(o1);
      ...
      \node[greencard,text width=2.2cm] (o2) at (3.0,-1.25) {combine\\the views};
...
\begin{tikzpicture}
  \node[bluecard,text width=12.5cm,font=\large] {%
    \textbf{Doppler makes neighbour loops unfinished; partial FFT keeps enough short-time pieces to align the desired carrier while the ICI terms cancel more.}};
\end{tikzpicture}
\end{frame}
