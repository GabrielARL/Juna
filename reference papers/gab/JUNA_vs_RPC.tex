...
\begin{frame}{1. System model}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.46\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.5em}
        ...
        \item Partial FFT gives $M$ partial-FFT views per carrier
      \end{itemize}
    \end{column}
    \begin{column}{0.52\textwidth}
      ...
      \[
        \vY_k \;=\;
        \sum_{q\in\Bk}\;
        \underbrace{\vc_{k,q}}_{\substack{\text{\scriptsize local ICI / nonideality}\\\text{\scriptsize coefficient vector}\\\text{\scriptsize $(M\times 1)$}}}\;
  ...
  \begin{tikzpicture}[node distance=0pt]
    ...
    \node[branchbox] (br) at (8.5,0) {view 1\\view 2\\$\vdots$\\view $M$};
...
% =====================================================================
\begin{frame}{3a. Partial-FFT observation model}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[flowbox,minimum width=8.4cm,minimum height=1.6cm]
                                   (jb)  at ( 0.0,-1.7) {Joint Estimation \& Inference\\
                                                          over $\vC$, $\vW$, $\vz$\\
                                                          {\footnotesize\itshape\color{inkgray}(partial-FFT observation model + pilots + LDPC)}};
...
\begin{frame}{5. The combining weight}
  ...
  \begin{center}
    ...
    symbols jointly. Scalar combining maps the vector of partial-FFT views to one number
    before decoding.}
  \end{center}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.25em}
        ...
        \item Partial FFT turns time variation into $M$ partial-FFT views:
              \[
	                H_i(t)\quad\leadsto\quad
	                \big[H_i(1),\ldots,H_i(M)\big].
	              \]
        \item Differences among the views are the degrees of freedom
      ...
      \begin{tikzpicture}[x=1cm,y=1cm]
        \node[draw=ink,fill=paper,line width=0.6pt,
          ...
          {$\vY_k$\\{\footnotesize partial-FFT views}};
        \node[draw=rpcred,fill=redbg,line width=0.8pt,
      ...
      \begin{center}
        \begin{tikzpicture}
          \node[draw=ink,line width=0.6pt,fill=paper,
            ...
            {\footnotesize Desired terms add coherently;\\
             \footnotesize ICI terms from other carriers are partially nulled.};
        \end{tikzpicture}
      \end{center}
    \end{column}
  \end{columns}
...
% =====================================================================
\begin{frame}{5b. Partial-FFT combining}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={font=\footnotesize}]
    ...
    \node[draw=rpcred,fill=redbg,line width=0.8pt,
      ...
      {Doppler ICI\\[-1pt] $d_{k-1}\vc_{k-1,k}$};
    \node[draw=rpcred,fill=redbg,line width=0.8pt,
    ...
    % Partial-FFT-view-space box
    \node[draw=ink,fill=paper,line width=0.6pt,
          minimum width=4.2cm,minimum height=2.75cm] (spacebox) at (-0.55,0.05) {};
    \node[font=\bfseries] at (-0.55,1.75) {partial-FFT observations};
    ...
    \begin{scope}[shift={(-0.55,0.00)}]
      ...
      \node[font=\scriptsize\itshape\color{inkgray},align=center] at (0.55,-0.70)
        {different patterns\\across the views};
    \end{scope}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        ...
        \item Stack pilot observation vectors as rows:
      \end{itemize}
      \vspace{-0.5em}
      \[
        \bm U_k=
...
\begin{frame}{7. Scalar or vector}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      ...
      \[
        \hX_k \;=\; \vW_k^{\herm}\,\vY_k .
      \]
      Pilots train $\vW_k$; information across the views is discarded.
      ...
      \]
      Each $\vc_{k,q}\!\in\!\Cx^{M}$ is the ICI profile from $q$ across the $M$ partial-FFT views.
    \end{column}
  ...
  \begin{center}
    {\itshape\small Partial-FFT combining compresses $\vY_k$ before the code is used.\;
     JUNA models $\vY_k$ as a sum of nearby symbols times profiles across the partial-FFT views.}
  \end{center}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.44\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.22em}
        ...
        \item Each neighbor $q$ contributes ICI to partial-FFT view $m$
        \item Stack across $M$ partial-FFT views $\Rightarrow$ $\vc_{k,q}\in\Cx^{M}$
        \item ICI coefficients decay with $|k-q|$ $\Rightarrow$ \textbf{banded}
...
\begin{frame}{9. The mixing matrix}
  ...
  \begin{tikzpicture}[x=0.85cm,y=0.55cm,font=\footnotesize]
    ...
    \node[anchor=west,font=\scriptsize\itshape\color{inkgray},align=left] at (8.7,-4.3)
      {bandwidth $K_{\rm ICI}\!=\!1$\\(ICI from only nearest neighbors)};
...
\framesubtitle{one cost over $(\vC,\vW,\vz)$ --- six jobs in one place}
  ...
  \underbrace{\lambda_d\sum_{k\in\mathcal K}
    \big\|\vY_k-\sum_{q\in\Bk}\vc_{k,q}s_q(\vz)\big\|_{\Sigma_k^{-1}}^2}_{\textstyle\textcolor{rpcred}{\text{(A) observation fit}}}
  \;+\;
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,font=\scriptsize]
    \node[draw=rpcred,fill=redbg,line width=0.7pt,minimum width=2.4cm,minimum height=0.55cm,align=center,font=\scriptsize\bfseries] at (-6.0,0) {(A) observation fit};
  ...
  \begin{center}
    {\itshape\small Three unknowns $(\vC,\vW,\vz)$, six terms; pilots, code, and the partial-FFT observation model regularize one another.}
  \end{center}
\end{frame}
...
\begin{frame}{12. The six terms}
  ...
  \begin{tabular}{|>{\columncolor{labelbg}\bfseries\footnotesize}p{2.5cm}
    ...
    \hline
    \textcolor{rpcred}{(A) observation fit}
    ...
    \begin{tikzpicture}
      \node[draw=ink,fill=paper,line width=0.6pt,minimum width=14cm,minimum height=0.95cm,align=center,inner sep=5pt]
        {Drop (A), (C), (E), (F): the $\vW_k$ update collapses to the pilot-trained ridge/RLS system.\\
         JUNA $=$ Partial-FFT combining \emph{plus} the partial-FFT observation model, soft-symbol coupling, smoothness, and code.};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{13. Alternating Solver}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.49\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35em}
        \item Fixed $\vz$: update $\vC$ by ridge least squares on the partial-FFT observation model.
...
\begin{frame}{14. Comparison}
  ...
  \begin{tabular}{|>{\columncolor{labelbg}\bfseries}p{2.7cm}
    ...
    Model in use
      ...
      & Local vector model in the partial-FFT domain \\
    \hline
    What is estimated
...
\begin{frame}{16. Information flow}
  ...
  \begin{tikzpicture}[x=1cm,y=1cm,scale=0.92,every node/.style={transform shape}]
    ...
    \node[font=\scriptsize\itshape\color{rpcred}] at (-0.7,-1.15)
      {Information across the views is no longer visible to decoder};
...
\begin{frame}{17. Takeaway}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35em}
    ...
    \item While scalar combining reduces ICI, it hides information across the partial-FFT views
