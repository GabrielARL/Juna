...
\begin{frame}{7. Scalar or vector}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      \textbf{\color{rpcred}RPC view.} Compress $\vY_k$ to one number, then decode:
      \[
        \hX_k \;=\; \vW_k^{\herm}\,\vY_k .
      \]
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.44\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.22em}
        ...
        \item RPC cancels these terms; JUNA \emph{models} them
      \end{itemize}
    \end{column}
...
\framesubtitle{(Information-Theoretic View)}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25em}
    \item RPC forms $\hX = g(\vY)$, then decodes
...
\begin{frame}{17. Takeaway}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35em}
    \item RPC splits the symbol, then combines with $\vW_k$
