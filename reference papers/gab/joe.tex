...
reject a refinement. Selected data anchors enter the reported RLS update with the
same sample weight; the confidence value is used for gating/ranking, not as a
per-sample RLS multiplier. In the reported benchmark profile $c_{\min}=0$, so the
