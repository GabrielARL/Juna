...
\item We derive two bounded receivers from it (Section~\ref{sec:solver}).
  ...
  anchor-count and iteration limits. Its steps are partial-FFT RLS seed, BP
...
Doppler leakage without altering the transmit waveform~\cite{wang2012fdoversampling}.
...
interference. \juna{} does not eliminate the need for a CP. However, code
...
\node[box, fill=labBlueLight, draw=labBlue, minimum width=37mm] at (6.3,-4.65)
  {$682$ QPSK data tones\\[-1pt]\tiny 680 carry $1360$ coded bits};
...
\node[box, fill=labGoldLight, draw=labGold, minimum width=18mm, right=2mm of msgAnc] (parity)
  {$1020$ parity\\[-1pt]\tiny $\mathbf v=\Hm\mathbf u$};
\node[note, anchor=west] at (0.8,-6.85)
  {message row $k=340$: violet positions are clamped before BP; parity completes $n_b=1360$};
...
\node[title, anchor=west] at (10.0,-0.6)
  {Tanner graph slice: $\Hc=[\Hm\ \mathbf I_m]$, $k=340$, $m=1020$};
...
has $1023$ active tones per symbol, split into $341$ outer frequency-domain
pilots and $682$ QPSK data tones. Of these, $680$ data tones carry the coded
bits and the remaining two are fixed to one. The logical FEC layer maps $k=340$
systematic message bits and $m=1020$ parity bits to $n_b=1360$ coded bits. Inner pilots are
...
\Require Partial-FFT views, pilots, LDPC code, budgets \(I_{\max},A_{\max}\),
  threshold \(c_{\min}\)
\State Train/apply the pilot-only Partial-FFT RLS seed.
...
decodes and \juna{} ties. On impossible channels, the seed posterior is
...
Both deployed receivers start from the Partial~FFT+FEC seed and run a bounded
...
packets can terminate at the seed decode or after an early payload-exact pass;
...
combining. \textbf{\junalite{}+FEC} uses the partial-FFT seed, posterior
...
acceptance or the seed posterior is effectively uninformative. A
...
Partial-FFT RLS bands & 4 frequency bands in these benchmark runs;
implementation default: 16 \\
...
selection is not the sole source of the gain. (iii) We give the Partial-FFT seed
