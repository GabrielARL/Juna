...
%
% Measured tables below carry the accepted benchmark numbers from the profiled
% workbench runs. Controlled/native table rows are archived in
% benchmark_results.csv; SG/NA/HW tuning tables \input generated fragments from
% ../../tex/figure_data.
%=============================================================================
...
partial-FFT RLS combiner as soft data anchors and reuses the front end. This
recovers most of the gain at a fraction of the cost and makes the
measured-channel study tractable. \junalite{}+FEC improves packet success over
Partial~FFT+FEC in hard-but-decodable measured channels. It gives substantial
gains on the controlled red channel and the hardest native capture, ties on
ceiling channels, and fails with the baselines on floor channels. In stacked
head-to-head comparisons, \junawz{} is usually at least as good as \junalite{}.
It has disjoint Wilson-interval wins and no disjoint losses, with a few small
non-significant regressions on near-floor NA cells. A cross-channel tuning study
shows that FFT size is usually a high-impact retuning knob. Conservative
modulation and code-rate choices then keep the receiver inside the decodable
regime.
\end{abstract}
...
interval. Even after packet acquisition and initial resampling, residual
...
time. It computes several partial-FFT observations per carrier and learns a
local bandwise combiner by recursive least squares (RLS) on known pilots. This
...
partial-FFT observation vectors and a frozen ICI model. It jointly moves the
combiner and the soft bit latents toward the pilots and the code. The LDPC
belief-propagation (BP) posterior supplies the code information. This exchange
is close to turbo equalization~\cite{douillard1995turbo,
tuchler2002turbo,jhuang2011progressive}, but its architecture is narrower
(Section~\ref{sec:related}). \juna{} keeps the partial-FFT observation vectors
and feeds decoder-weighted information back into that front end. It does not
replace the front end with a generic soft-in/soft-out equalizer. A lightweight
variant, \junalite{}, approximates the same loop without forming a gradient. It
maps the posterior LLRs to posterior-mean constellation points and re-estimates
the partial-FFT RLS combiner with these soft training anchors. A decoded data
symbol with a useful posterior can therefore behave like a \emph{soft pilot}.
The LDPC parity graph supplies structure beyond the physical pilot tones and can
share the equalizer-training burden. Because \junalite{} is cheap enough to run across
...
\item A coupled estimation/decoding objective (Section~\ref{sec:juna}) in which
  ...
  one another.
\item We derive two bounded receivers from it (Section~\ref{sec:solver}).
  \junawz{} descends the reduced $(\mathbf W,\mathbf z)$ objective with
  confidence-weighted updates and optional BP projection. \junalite{}
  approximates the same loop as a posterior-anchor RLS refit with explicit
  anchor-count and iteration limits. Its steps are partial-FFT RLS initial candidate, BP
  posterior LLRs, posterior-mean soft data anchors, combiner refit, and
  candidate selection. The full $(\mathbf W,\mathbf C,\mathbf z)$
  solver, \junawcz{}, is reported as a diagnostic rather than as the deployed
  sweep receiver.
...
explain the physical source of the problem studied here. However, they seek
explicit channel or ICI estimates. \junawz{} instead descends a code-aware
reduced objective over an empirical partial-FFT combiner and the data symbols
using decoder-derived confidences. Its \junalite{} variant retrains that
combiner from decoder-derived soft targets.
...
signal-division multiplexing (D-OSDM) designs the data/pilot vector layout under
a basis-expansion model of the doubly spread channel. Its block structure is
preserved after propagation, and the Doppler-induced coupling can be inverted at
the receiver~\cite{ebihara2016dosdm}. A second group changes the observation or
estimation basis rather than the modulation. Frequency-domain oversampling of
zero-padded OFDM sharpens channel/ICI estimation and reduces sensitivity to
Doppler-induced ICI without altering the transmit waveform~\cite{wang2012fdoversampling}.
Wavelet/filter-bank representations have also refined channel estimation under
Doppler spread~\cite{tang2018haarwavelet}. These methods share \juna{}'s
...
linear combiner from pilots. \juna{} keeps this observation-vector
representation. The decoder posterior either weights a reduced gradient update
of the combiner and the soft data symbols (\junawz{}) or provides soft targets
for a bounded anchored RLS refit (\junalite{}). Pilot-aided ICI mitigation outside UWA
...
\junawz{} update of the partial-FFT combiner and soft data symbols. In
\junalite{}, decoder \emph{posterior means} become soft data pilots that retrain
the RLS combiner. Both paths use anchor or iteration caps, optional confidence
thresholding, and decoder-grounded candidate selection. A
...
\node[ours] (a4) at (3.1,-0.9) {Partial-FFT\\evidence\\initial-candidate LLRs};
...
information. Partial-FFT observation-vector evidence produces initial-candidate LLRs, LDPC BP returns
...
$a$ the residual Doppler factor after initial resampling. This single-scale form is a
...
assume one global scale. The origin of ICI is already visible in a stripped-down
...
a residual Doppler factor leaves a term of the form
\begin{equation}
...
columns contribute ICI to row $k$ of the frequency-domain channel matrix. Fig.~\ref{fig:ici-origin-matrix}
...
For pure time scaling the magnitude of the ICI coefficient is a Dirichlet kernel in
...
that initial resampling has made the residual Doppler factor small over the active
...
observation vectors. It learns a local scalar combiner from pilots and
decoder-derived soft anchors.
...
\begin{equation}
  ...
  \label{eq:cp-short}
\end{equation}
where $\mathbf H_0$ is generally non-diagonal and maps carriers within OFDM
symbol $s$, while $\mathbf J_-$ and $\mathbf J_+$ map OFDM symbols $s-1$ and
$s+1$ into $\mathbf Y_s$, respectively, and $\bm\nu_s$ is noise. For a causal
channel and an FFT window aligned with the useful interval,
$\mathbf J_+=\mathbf 0$; the general model retains this term when the window
contains samples from the next OFDM symbol. An off-diagonal entry of
$\mathbf H_0$ is an ICI coefficient because its carrier indices differ while
its OFDM symbol index is unchanged. An entry of $\mathbf J_{\pm}$ is
intersymbol interference (ISI) because its transmitted and received OFDM
symbol indices differ. An insufficient CP can therefore produce both ICI
within one OFDM symbol and ISI from the preceding OFDM symbol. \juna{} does not
eliminate the need for a CP. However, code
constraints can help suppress residual coupling within the partial-FFT
combiner's effective model class instead of relying only on pilots.
...
reported implementation, one combiner per frequency band is trained over the
...
replaces the pilot-stacked desired response with a packet-level forward model
and adds a differentiable parity penalty. Pilots, partial-FFT observations, the
combiner, and the code constraints then regularize one another within a single
...
Fig.~\ref{fig:packet-tanner-example} reproduces the workbench packet/Tanner
layout for a concrete configuration,
\cfg{n1024\_cp256\_sym1\_p3\_r0p25\_dc4\_sig1\_ip2}. The left panel separates
...
encoding and modulation. The right panel shows the corresponding Tanner-graph
slice for the systematic implementation $\Hc=[\Hm\ \mathbf I_m]$. The suffix
\cfg{dc4} is a configuration sparsity knob $d_{\rm cfg}=4$; at this rate the
constructor maps it to $d_v=6$ message-column edges in the Tanner graph.
...
\node[title, anchor=west] at (0.8,-0.6)
  {Packet structure: \cfg{n1024\_cp256\_sym1\_p3\_r0p25\_dc4\_sig1\_ip2}};
...
\node[box, fill=labBlueLight, draw=labBlue, minimum width=37mm] at (6.3,-4.65)
  {$681$ QPSK data tones\\[-1pt]\tiny carry $1362$ coded bits};
...
\node[box, fill=labGoldLight, draw=labGold, minimum width=18mm, right=2mm of msgAnc] (parity)
  {$1022$ parity\\[-1pt]\tiny $\mathbf v=\Hm\mathbf u$};
\node[note, anchor=west] at (0.8,-6.85)
  {message row $k=340$: violet positions are clamped before BP; parity completes $n_b=1362$};
...
\node[title, anchor=west] at (10.0,-0.6)
  {Tanner graph slice: $\Hc=[\Hm\ \mathbf I_m]$, $k=340$, $m=1022$};
\node[note, anchor=west] at (10.0,-1.12)
  {$d_v=6$ message-column edges; each parity node has one identity edge};
...
\node[note, text=labGold] at (16.1,-5.3) {parity tail};

% Edges from sparse H_m.
...
\node[box, fill=white, draw=labLine, minimum width=24mm, anchor=west] at (10.0,-7.35)
  {$\Hm$ sparse\\[-1pt]\tiny random message edges};
\node[box, fill=white, draw=labLine, minimum width=25mm, anchor=west] at (13.1,-7.35)
  {$\mathbf I_m$ parity\\[-1pt]\tiny one edge per check};
...
\caption{Workbench-style packet-structure and Tanner-graph example for
\cfg{n1024\_cp256\_sym1\_p3\_r0p25\_dc4\_sig1\_ip2}. The physical OFDM layer
has $1022$ active tones per symbol, split into $341$ outer frequency-domain
pilots and $681$ QPSK data tones. The logical FEC layer maps $k=340$ systematic
message bits and $m=1022$ parity bits to $n_b=1362$ coded bits. Inner pilots are
...
variables form the identity tail of $\Hc=[\Hm\ \mathbf I_m]$. The illustrated
layout is QPSK-specific: for BPSK, the same mapping is used but each data tone
carries one coded bit, so $n_b=N_d$ rather than $2N_d$.}
...
hard-decision decoder~\cite{richardson2008modern}. The implementation uses the
systematic construction $\Hc=[\Hm\ \mathbf I_m]$. It maps $k$ message bits to
$n_b=k+m$ coded bits, with rate $k/n_b$ set by the modem configuration. The
configured sparsity knob $d_{\rm cfg}$ determines the sparse message-bit degree.
For example, the workbench suffix \cfg{dc4} means $d_{\rm cfg}=4$; under the
rate-$0.25$ rule used for Fig.~\ref{fig:packet-tanner-example}, this becomes
actual message-column degree $d_v=6$. Some message
...
The identity parity tail makes $\Hc$ full row rank by construction and gives a
systematic encoder: for message vector $\mathbf u$, the parity suffix is
$\mathbf v=\Hm\mathbf u \pmod 2$, so $[\mathbf u,\mathbf v]$ satisfies
$\Hc\mathbf b\T=\mathbf 0$. This code is intentionally a simple, deterministic
workbench sparse parity code rather than an optimized acoustic LDPC design. The
fixed packet/configuration seed generates the random sparse message part. All
receiver variants in a comparison use the identical code instance. We therefore
interpret the measured gains as receiver-level gains under this fixed sparse
code.

Because the code constraints are binary while the subcarriers carry QPSK, the bit
and symbol variables are kept separate. The \junawz{} and \junawcz{} code paths
use $\xi_i=\tanh(z_i/2)\in(-1,1)$ as the relaxed bipolar variable for coded bit
$b_i$. Hard bits satisfy $\xi_i=1-2b_i$. For Gray-labeled QPSK, data carrier $k$ carries bits
...
This surrogate is a differentiable consistency penalty in the objective and
gradient diagnostics. The implemented receiver relies on BP for the actual code
update because direct optimization of high-degree parity products can be poorly
conditioned and can have weak gradients.
...
\begin{equation}
  ...
  \sum_{(p,s)\in\mathcal P_b}
      \lambda_{\rm RLS}^{\tau_{p,s}}
      \abs{P_{p,s}-\mathbf w\Ht\mathbf y_{p,s}}^2\\
  &+\rho\sum_{(d,s)\in\mathcal D_b^+}\omega_{d,s}
      \lambda_{\rm RLS}^{\tau_{d,s}}
      \abs{\widetilde x_{d,s}-\mathbf w\Ht\mathbf y_{d,s}}^2\\
  &+\delta\norm{\mathbf w}_2^2 ,
  \end{aligned}
  \label{eq:soft-pilot-ls}
\end{equation}
where $\mathcal D_b^+$ is the selected data-anchor set, $\omega_{d,s}$ is an
optional anchor weight, $\tau$ is RLS update age ($\tau=0$ for the most recent
sample), and $\lambda_{\rm RLS}$ is the forgetting factor. The measured receiver
sets $\omega_{d,s}=c_{d,s}$ for selected anchors. Confidence also gates and
ranks candidate anchors when a threshold or cap is active, while uncertainty in
the target is expressed by the posterior mean $\widetilde x_{d,s}$. Compared with
\eqref{eq:pfft-ls}, the normal matrix gains selected data-anchor terms
$\sum_{(d,s)\in\mathcal D_b^+}\omega_{d,s}\mathbf y_{d,s}\mathbf y_{d,s}\Ht$.
When the decoder posterior is informative, those terms improve conditioning as
additional pilots would. When it is uninformative or biased, retained physical
pilots and posterior means rather than hard slices limit the damage. Anchor caps,
optional confidence thresholding, candidate selection, and early stopping add
further limits. In the profiled QPSK benchmark, $c_{\min}=0$ disables the
confidence threshold. Confidence then affects cap-based ranking only when the
cap binds.
...
not an identifiable physical channel estimator. The partial-FFT-domain response
tensor $\mathbf C$ and combiner $\mathbf W$ are coupled through the symbols,
pilots, and regularizers. However, no explicit constraint forces $\mathbf W$ to
be the inverse of $\mathbf C$. Its role is to show how observation-vector consistency, combiner
...
neighborhood. To include ISI caused by an insufficient CP, we extend the
carrier index $k$ to $(k,s)$ and replace $\mathcal B_K(k)$ with a local
space-time neighborhood containing neighboring OFDM symbols:
\begin{equation}
  \mathbf Y_{k,s}
...
same-symbol ICI and cross-symbol ISI. This is a
...
front-end fit. The measured results below do not validate a CP-relaxation rule.

\subsection{Relation to the Pilot-Trained Combiner}\label{ssec:juna-relation}
\juna{} extends~\eqref{eq:pfft-ls}. The packet-level partial-FFT-domain model
replaces the pilot-only desired response, and every carrier refines the front
end. Bit latents generate the data field so soft bits improve during inference.
The code enters through~\eqref{eq:soft-parity}. Equation
\eqref{eq:juna-total} is best read as a theoretical lens---it is not the exact
algorithm run on measured channels. In particular, the at-scale receivers do not
estimate the full partial-FFT-domain response tensor $\mathbf C$ or the banded ICI matrix;
those variables make explicit the physical coupling that motivates the update.
...
%=============================================================================
\section{Implemented Receivers: \texorpdfstring{\junawz{}}{JUNA-Wz} and
\texorpdfstring{\junalite{}}{JUNA-lite}}\label{sec:solver}
We implement two measured receivers derived from the coupled objective. The
...
Its coordinates are weighted by decoder confidence and can be projected
periodically through the LDPC BP decoder (Section~\ref{ssec:gradient-juna}).
...
gradient descent with a bounded alternating posterior-anchor RLS refit
...
\junalite{} costs a fixed number of RLS/BP passes per packet and is cheap enough
to run across every measured capture, so it carries the large-scale sweeps;
...
\junawcz{}, is a diagnostic path described separately.
Fig.~\ref{fig:juna-iterative-loop} gives the receiver loop as a pictorial
summary: the top row is the Partial-FFT/FEC initial-candidate path, and the dashed paths are
...
\subsection{OFDM5 Prototype Solvers: Reproducible Update Forms}
\label{ssec:ofdm5-prototypes}
For clarity, we also record the optimization paths in the earlier OFDM5
prototype. These paths are useful implementation lineage, but they are not the
same claim as the measured \junawz{}/\junalite{} receivers above. OFDM5 contains
...
solver. It first runs an initial-candidate block-coordinate pass, forms data-bin LLRs from the
...
placeholder in this mode. Downstream code uses \(H(\mathbf h)\), the same
\(\mathbf x^{\rm fixed}\) blend, and the optional BPSK code refiner. It does not
run the Richardson demixing stage.
...
\footnotesize
\begin{algorithmic}[1]
\Require Partial-FFT observations \(\mathbf Y_p\) with \(V\mid N\), pilots, anchor
...
\node[feedbackbox, below left=8mm and 7mm of post] (lite) {\textbf{\junalite}\\
  soft anchors\\RLS combiner refit};
...
first runs the ordinary Partial-FFT/FEC initial-candidate path. LDPC BP then returns
...
\end{equation}
with the obvious constellation-expectation generalization for BPSK/8PSK/16PSK.
...
and reruns the same RLS update with desired responses equal to the known pilots
on $\mathcal P_b$ and the posterior means $\widetilde x_{k,s}$ on the selected
data anchors. The default receiver therefore trains toward posterior means---not
hard decisions---keeps pilots in the anchor set, caps the data anchors, and can
reject a refinement. Pilots enter the reported RLS update with unit weight.
Each selected data anchor enters with a weight equal to its confidence
$c_{d,s}$. Hence a less certain anchor pulls the fit less than a pilot or a
confident anchor. In the reported benchmark profile $c_{\min}=0$, so the
confidence threshold itself is disabled; when the anchor cap binds, confidence is
used to rank anchors, and otherwise all data tones are eligible.

\begin{algorithm}[t]
\caption{\junalite{} posterior-anchor RLS refinement}
\label{alg:junalite}
\footnotesize
\begin{algorithmic}[1]
\Require Partial-FFT views, pilots, LDPC code, budgets \(I_{\max},A_{\max}\),
  threshold \(c_{\min}\)
\State Train/apply the pilot-only Partial-FFT RLS combiner.
\State Demap to channel LLRs, run LDPC BP, and store the initial candidate.
\For{\(i=1,\ldots,I_{\max}\)}
  ...
  \For{each RLS band \(b\)}
    ...
    \State Refit the band combiner toward pilots and posterior means.
  \EndFor
...
Each pass decodes the candidate symbols and compares validity first. In strict
mode, a valid LDPC codeword beats an invalid one, and lower syndrome weight
breaks ties. Remaining ties use pilot MSE and the confidence-weighted distance
from equalized symbols to posterior means. The score also uses normalized
syndrome weight and a small reward for sharper posteriors:
\begin{equation}
  S_{\rm cand}
...
$\overline{|L^{\rm post}|}$ is the mean posterior-LLR magnitude. In the
deployed receiver $M_{\rm pilot}$ is inactive and takes the same value for
every candidate: residual pilot equalization divides each pilot carrier by a
response measured on that carrier, so $\widehat x_{p,s}=P_{p,s}$ to numerical
precision. The score therefore separates candidates through $M_{\rm tie}$,
$w_{\rm synd}$, and $\overline{|L^{\rm post}|}$. We retain $M_{\rm pilot}$ in
the reported form because it would separate candidates before residual pilot
equalization. Lower
...
\end{equation}
The initial candidate $(\mathbf W_0,\mathbf z_0)$ comes from the partial-FFT/\junalite{}
candidate and its channel or posterior LLRs, $\mu$ is a trust weight to prevent
large moves away from the initial candidate, and
...
uncertain ones. The \texttt{wz\_juna\_bp} variant periodically re-decodes the
refined logits and uses the BP output as a projection/checkpoint; the default
\texttt{wz\_juna} receiver instead selects the best decoder-grounded checkpoint
across the descent. For offline diagnostics only, the harness can also record an
oracle non-regression choice from known payload mismatch. The receiver tables do
not use this oracle choice. They rely on syndrome, CRC-surrogate acceptance, or
the score in~\eqref{eq:candidate-score}. \junalite{}
...
\footnotesize
\begin{algorithmic}[1]
\Require Partial-FFT views, pilots, LDPC checks, initial candidate
  \((\mathbf W_0,\mathbf z_0)\), confidences \(c_{d,s}\), step budget \(G\)
\State Initialize \((\mathbf W,\mathbf z)\gets(\mathbf W_0,\mathbf z_0)\).
\State Store the initial or \junalite{} candidate as the current best.
\For{\(g=1,\ldots,G\)}
  ...
  \If{\(g\) is a BP checkpoint}
    \State Map \(\mathbf z\) to LLRs, run BP, and score the checkpoint.
    \State Update the best candidate by decoder-first selection.
  \EndIf
\EndFor
\State Decode the final checkpoint if it was not already checked.
\State \Return the best decoder-grounded candidate.
\end{algorithmic}
\end{algorithm}

\subsection{\texorpdfstring{\junawcz{}}{JUNA-WCz}: Full-Objective Diagnostic}
...
joint Adam steps. The deployed \texttt{wz\_juna} receiver fixes $\mathbf C$ to the
front-end model and descends only $(\mathbf W,\mathbf z)$. The full diagnostic
loss contains the partial-FFT observation-consistency term, pilot-anchor term,
combiner--symbol tie term, LDPC parity surrogate, and trust/regularization
terms. The extra $\mathbf C$ degrees of freedom are numerous and only weakly
identifiable from sparse pilots. We therefore exclude this solver from the
large-scale receiver claim. It checks whether opening the full objective moves
the same decoder metrics in the same direction on a controlled segment.
...
disables the usual non-regression gate. Hence the \junawcz{} row is the raw
optimizer output rather than ``best of \junalite{} and \junawcz{}.'' The
full-objective optimizer uses an eight-step gradient budget and ICI
half-bandwidth $K_{\rm ICI}=1$. It matches \junalite{} on valid packet count
($1/11$ for both) and differs by only two message-bit errors out of $7491$.
This single segment is a diagnostic check, not a replacement for the
measured-channel comparisons of Section~\ref{sec:results}. We read it as
evidence that opening the full objective does not degrade decoder metrics on a
controlled stress segment. It is not evidence of a \junawcz{} win over
\junalite{}.
...
training set would reduce the variance of the RLS estimate,
...
not proof: the anchors are decoder-processed functions of the same noisy initial candidate and
are correlated with it. The gain comes from correlated but shrunken constraints
when the posterior is informative. Posterior means, anchor caps, retained pilots,
validity/syndrome selection, and early stopping limit the effect of posterior
bias. This gives three empirical regimes. On easy channels, Partial~FFT already
decodes and \juna{} ties. On impossible channels, the initial-candidate posterior is
uninformative and \juna{} cannot help. On hard-but-decodable channels, the
posterior is informative but not decisive. Posterior anchors can then move the
...
RLS refits, plus decoder-grounded BP checks.
...
of OFDM data symbols in a frame, and $N_a$ the number of active subcarriers. Let
$N_p$ be the number of outer pilot tones and $N_d=N_a-N_p$ the number of data
tones. Let
$N_{\rm view}$ be the number of partial-FFT views, $B$ the number of RLS frequency
...
gradient refinement and $C_{\rm ckpt}$ the number of BP checkpoint/projection
decodes during that descent. For \junawcz{}, let $K$ be the ICI half-band used in
...
\subsection{Partial-FFT RLS Front End}
...
benchmarked here. Bandwise RLS training updates a length-$N_{\rm view}$ combiner using the
outer pilots. One RLS update requires a matrix-vector multiply and rank-one
inverse-covariance update, both $O(N_{\rm view}^2)$. Training therefore costs
...
\end{equation}
For the reported runs, $N_{\rm view}=4$ and $B=4$, so the RLS matrices are small.
The main cost increase over one-tap OFDM is the fixed number of extra FFT views
plus the LDPC decode shared by all FEC receivers.

\subsection{Per-Pass Refinement Cost}
Both deployed receivers start from the Partial~FFT+FEC initial candidate and run a bounded
number of passes. \junalite{} converts LDPC posterior LLRs into soft symbols,
selects data anchors, refits the same bandwise RLS combiner using pilots plus
...
all-anchor path when $A_{\max}$ covers the data tones. The RLS refit costs
...
the experiment settings: $I_{\max}=4$, $A_{\max}=1024$, $N_{\rm view}=4$, and $B=4$. Easy
packets can terminate at the initial decode or after an early payload-exact pass;
floor packets still pay at most the fixed iteration budget.

\junawz{} replaces the RLS refit with $G$ Adam steps on
...
\begin{aligned}
  ...
  ={}&T_{\rm PF}
   ...
   &\hspace{2.2em}+(C_{\rm ckpt}+1)T_{\rm BP}E\big),
\end{aligned}
  \label{eq:wz-complexity}
\end{equation}
where the extra decode covers the final decoder-grounded selection when it is
not already one of the checkpoints. With fixed $G$, fixed checkpoint cadence, and
...
\junawz{}+FEC &
$T_{\rm PF}+O(G(SN_aN_{\rm view}+SN_d+E)+(C_{\rm ckpt}+1)T_{\rm BP}E)$ \\
\junawcz{} diagnostic &
$T_{\rm PF}+O(G_{\rm full}(SN_a(2K+1)N_{\rm view}+SN_d+E))$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
observations, $O(SN_{\rm view}N_a)$, plus bandwise weights and inverse covariances,
$O(BN_{\rm view}+BN_{\rm view}^2)$. \juna{} adds the current/best equalized candidates and posterior
LLRs, $O(SN_a+n_b)$, but reuses the LDPC scratch buffers and RLS state across
...
We evaluate three receivers on the same transmitted schedule, channel replay,
SNR, FEC, and acceptance rule. \textbf{OFDM+FEC} uses one-tap pilot-linear
equalization, while \textbf{Partial~FFT+FEC} uses bandwise partial-FFT RLS
combining. \textbf{\junalite{}+FEC} uses the partial-FFT initial candidate, posterior
extraction, posterior-anchor RLS refinement, re-decode, and candidate selection.
This lightweight \juna{} receiver carries the sweeps. Packet success rate (PSR) is
...
Configuration identifiers encode the modem geometry and code. For example,
\cfg{n1024\_cp256\_sym1\_p3\_r0p25\_dc4\_sig1\_ip2} means $N=1024$, CP length
$256$, and one OFDM payload symbol per frame. It uses one outer pilot every
three active subcarriers and LDPC design rate $0.25$. The LDPC sparsity knob is
$d_{\rm cfg}=4$, encoded in the suffix as \cfg{dc4}. The FEC LLR scale floor is
$1$, with one inner-pilot message bit every two message positions. For the
cross-channel tuning table, the coordinate search at 20\,dB sweeps FEC rate,
outer/inner pilot spacing, and $d_{\rm cfg}$. It then sweeps the joint
$(N,\mathrm{CP})$ grid and modulation. Each stage fixes the best JUNA PSR, with
BER as the tie-breaker, before moving to the next stage. The OAT table then perturbs one factor at a
...
We call a channel \emph{ceiling} when Partial~FFT+FEC is already above roughly
$0.95$ PSR. We call it \emph{floor} when all receivers remain near zero packet
acceptance or the initial-candidate posterior is effectively uninformative. A
\emph{hard-but-decodable} channel has useful posterior confidence but still
fails payload acceptance on a substantial fraction of packets. These labels
describe the measured results; they are not pre-deployment classifiers.
...
\begin{table}[t]
\centering
\caption{Receiver hyperparameters for the profiled benchmark runs.}
...
20\,dB. \junalite{}+FEC gives large gains on the hard \texttt{red\_1} and
selected \texttt{blue\_1} captures. It gives a one-packet near-ceiling gain on
\texttt{yellow\_1}. The rescued packets have substantially lower message BER,
so they are better decoded rather than merely re-thresholded.
...
Table~\ref{tab:native20} reports 20\,dB. The gain is large on
hard-but-decodable \texttt{red\_1} ($18/190\to135/190$) and the selected
\texttt{blue\_1} ($35/110\to90/110$). It is near ceiling on \texttt{yellow\_1}
and \texttt{purple\_1}. All three receivers fail on the floor channel
\texttt{green}.
...
selection is not the sole source of the gain. (iii) We give the Partial-FFT initial candidate
the same LDPC decode-call budget as \junalite{}, without posterior anchors. The
result is numerically identical to ordinary Partial~FFT in the deterministic
replay path. This mainly rules out ``more decode calls'' as the mechanism. Multi-seed numeric
...
\junawz{} matches or exceeds \junalite{} at every displayed point. The largest
margins occur at low SNR, where SG-1 rises from $0.09$ to $0.86$ at $0$\,dB.
Payload-exact PSR then reaches $1.0$ as SNR grows. These displayed cases are
the ceiling-reaching SG-1/NA-1/HW-1 captures;
...
as \junalite{} in $44$. It is strictly better with disjoint Wilson 95\%
intervals in $18$ and never significantly worse.

The gain is not an artefact of the two gradient guardrails.
Table~\ref{tab:gradient-ablation} replaces the soft confidence weighting with
hard anchoring. It also takes the final iterate instead of the decoder-grounded
checkpoint. Payload-exact PSR remains unchanged to within one packet on the
representative captures. Hence the improvement comes from decoder-aided descent,
not from confidence weighting or selection alone.
...
three physically distinct sites. Singapore harbour provides
\texttt{SG-1/2/3}, shallow at 25\,kHz. The North Atlantic provides
\texttt{NA-1/2/3}, mobile deep-water at 13\,kHz. Hawaii provides
\texttt{HW-1}\,$\dots$\texttt{6}, moored deep-water at 13\,kHz. For each capture,
we run a coordinate search at 20\,dB and one-factor-at-a-time (OAT) sensitivity
sweeps at 10/20/30\,dB. Fig.~\ref{fig:cir}
...
The OAT table reports mean PSR ranges from coarse one-factor screens. Equal or
nearly equal FEC and modulation swings indicate saturation or local sensitivity
ties, rather than evidence that the two axes are causally identical.
...
\small
\resizebox{\textwidth}{!}{\input{../../tex/figure_data/tab_best.tex}}
\end{table*}
...
\small
\resizebox{\textwidth}{!}{\input{../../tex/figure_data/tab_oat.tex}}
\end{table*}
...
modulation-sensitive and should not be presented as universally solved. The
...
the code--channel feedback principle explicit. \junawz{} uses reduced
confidence-weighted gradient descent over $(\mathbf W,\mathbf z)$, while
\junalite{} recovers most of the value without forming a gradient. \junalite{}
reuses partial-FFT RLS, extracts LDPC posterior means, feeds soft anchors back
into the equalizer, and accepts only refinements that improve decoder-grounded
criteria, which is what makes the twelve-capture study tractable. On measured
replay channels, this substantially beats Partial~FFT+FEC in the
hard-but-decodable regime. It preserves ties and failures on ceiling and floor
channels. The twelve-capture study gives a two-stage tuning rule that both
receivers broadly support. The scientific point is that decoder
