...
partial-FFT RLS combiner as soft data anchors and reuses the front
end---recovering most of the gain at a fraction of the cost, which makes a large
measured-channel study tractable. At
...
linear combiner---here a bandwise combiner trained by recursive least squares
(RLS) on known pilots. This baseline is far stronger than one-tap OFDM; in our
...
same partial-FFT RLS combiner with those means as cap-limited soft training
anchors. The key economy---shared by both---is that a decoded data symbol with a
...
\item Two bounded receivers derived from it (Section~\ref{sec:solver}):
  ...
  confidence-weighted updates and optional BP projection, and a lightweight
  \junalite{} that approximates the same loop as a posterior-anchor RLS refit
  (partial-FFT RLS seed $\to$ BP posterior LLRs $\to$ posterior-mean soft data
...
bounded anchored RLS refit (\junalite{}). Pilot-aided ICI mitigation outside UWA
...
pilots that retrain the RLS combiner---under anchor/iteration caps, optional
...
hard-decision decoder~\cite{richardson2008modern}. The implementation uses a
systematic construction $\Hc=[\Hm\ \mathbf I_m]$: $k$ message bits are mapped to
$n_b=k+m$ coded bits, with rate $k/n_b$ set by the modem configuration and sparse
message-bit degree chosen from the configured sparsity knob $d_{\rm cfg}$.
...
random sparse message part is generated from the fixed packet/configuration
seed, and all receiver variants in a comparison use the identical code instance;
therefore the measured gains should be interpreted as receiver-level gains under
this fixed sparse code.
...
sets $\omega_{d,s}=1$ for selected anchors; confidence $c_{d,s}$ gates and ranks
candidate anchors when a threshold or cap is active, while uncertainty is already
expressed by the shrunken posterior mean $\widetilde x_{d,s}$. Compared with
...
pilots, posterior means rather than hard slices, anchor caps, optional confidence
thresholding/ranking, and candidate selection limit the damage.
In the profiled QPSK benchmark, protection comes mainly from posterior means,
retained pilots, the fixed anchor cap, candidate selection, and early stopping:
$c_{\min}=0$ disables the confidence threshold itself, so confidence only affects
cap-based ranking when the cap binds.
...
reject a refinement. Pilots enter the reported RLS update with unit weight, and
each selected data anchor enters with a weight equal to its confidence
$c_{d,s}$, so a less certain anchor pulls the fit less than a pilot or a
confident anchor does. In the reported benchmark profile $c_{\min}=0$, so the
...
$w_{\rm synd}$, and $\overline{|L^{\rm post}|}$; $M_{\rm pilot}$ is retained in
the reported form because it would separate candidates for any receiver that
scored the combiner output before residual pilot equalization. Lower
...
oracle non-regression choice from known payload mismatch; this oracle choice is
not used in the receiver tables, which rely on syndrome, CRC-surrogate
acceptance, or the score in~\eqref{eq:candidate-score}. \junalite{}
...
terms. Because the extra $\mathbf C$ degrees of freedom are numerous and only
weakly identifiable from sparse pilots, this solver is not part of the
large-scale receiver claim; it checks whether opening the full objective can move
...
\end{equation}
For the reported runs, $N_{\rm view}=4$ and $B=4$, so the RLS matrices are small; the main
cost increase over one-tap OFDM is the fixed number of extra FFT views plus the
LDPC decode shared by all FEC receivers.
...
replay channels this is enough to beat Partial~FFT+FEC substantially in the
