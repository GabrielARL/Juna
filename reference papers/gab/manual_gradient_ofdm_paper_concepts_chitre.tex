...
descent.  However, many receiver variables are chosen rather than tuned:
the number of partial-FFT views, the intercarrier interference (ICI) span, the
sparse support of a channel, and the order in which interferers are cancelled
are all discrete.  Since a gradient carries no information about a discrete
...
vector that carries the coded bits, and show that the receivers in this paper
set differ only in the observation model that is substituted into it.  Although
the hand-derived gradient covers only the coded bits and a few low-dimensional
continuous refinements, it leaves the channel, the combiner, and the nuisance
terms to least squares, grid search, and proximal steps, which estimate them
without a gradient.
\end{abstract}

\section{A Receiver With Two Kinds of Variable}

The receivers in this paper set all recover coded bits from an observation that
depends on a channel, on receiver settings, and on whatever else the sea adds to
the signal.  We write the common problem as
\begin{equation}
...
holds the receiver physics such as Doppler, ICI span, channel coefficients and
partial-FFT combiners, and $\bm u$ holds nuisance terms such as impulsive noise
or external interference.  The first term measures how well the receiver
...
The receivers in this paper set observe the transmitted symbols through
different physics, but they all observe them linearly once the physical
variables are fixed.  We write the observation for group $a$ as
\begin{equation}
  \widehat{\bm y}_a
  =
...
Each receiver in this paper set supplies an observation model, and each one also
supplies a decision that we do not differentiate.  Table~\ref{tab:concepts}
lists both.
...
& The coded-bit latents, through the partial-FFT view model
$\bm Y_k\approx\sum_q \bm c_{kq}s_q(\bm z)$, using
\eqref{eq:universal_z_gradient}.
& The partial-FFT windows, the view count $M$, and the combiner weights.  We
...
Reliable soft symbols then act as decision-directed pilots.
...
\end{equation}
where the sum runs over the neighbours that are allowed to leak into carrier
$k$.  We minimize
\begin{equation}
  \mathcal L_{\rm pfft}
  =
...
therefore feels every carrier it leaks into, and the parity checks it takes part
in, at the same time.
...
\begin{enumerate}
  \item Generate candidates for coarse Doppler, partial-FFT view count $M$, and
  ICI span $K_{\rm ICI}$.
  \item For each candidate, initialize the channel and view coefficients from
  the pilots, with sparse support selection where it applies.
  \item Alternate:
  \begin{enumerate}
    \item update the channel, view and interference variables by least squares,
...
\begin{itemize}
  \item Integer quantities: the ICI span, the view count, the pilot pattern, the
...
physical-layer network coding all fit this pattern by changing the observation
matrix or the nuisance basis, while the coded-bit gradient of
