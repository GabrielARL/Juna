...
Partial FFT & Recompute \code{Ypartial} for each candidate $M$. Partial-FFT views are not merged in v1. \\
Windows & Rectangular, non-overlapping, partition-of-unity. Each sample contributes to exactly one partial-FFT view. \\
...
\code{CANDIDATE_M} & $[4,8,16,32]$ & Candidate values for the number of partial-FFT views. \\
...
\code{GAMMA_C} & $10^{-2}$ & Ridge for the local ICI coefficients in $C$. \\
...
\code{Ypartial} & $(N,M)$ & Partial-FFT views; row $k$, view $m$. \\
\code{W} & $(N,M)$ & Combiner weights. $\hat s_k=\mathbf w_k^H\mathbf Y_k$. \\
\code{C} & $(N,2K+1,M)$ & Local ICI coefficients. \\
...
q=k+off-K-1
\end{equation}
to received carrier $k$ in partial-FFT view $m$. Invalid $q$ values are skipped. No wraparound.
...
For $M$ partial-FFT views, view $m$ uses samples
\begin{equation}
...
\subsection{Known-Doppler phase for each partial-FFT view}

After CP removal, the midpoint time of partial-FFT view $m$ uses zero-based sample indices:
\begin{equation}
...
Requires \code{problem.truth !== nothing}. It computes partial FFT, combines the partial-FFT views with true Doppler phase, then runs pilot channel estimation and LDPC.
...
\item Initialize $C^{(0)}$ with only the diagonal coefficient for each partial-FFT view nonzero:
\begin{equation}
...
For candidate configuration $\theta=(M,K,\lambda_d,\eta,\rho,\gamma_C,\gamma_W,\gamma_S,\lambda_p)$, define the partial-FFT observation residual
\begin{equation}
...
For each received carrier $k$, let $\mathbf s_k=[s_q]_{q\in\mathcal B_K(k)}$. Let $C_k\in\C^{M\times B_k}$ collect valid coefficients for the partial-FFT views. Solve
\begin{equation}
...
Construct $Y_{k,m}=a_m X_k$ for all carriers with known partial-FFT observation vector $a$. Fit RPC on pilots. Verify $w^Ha\approx1$ and pilot residual below $10^{-10}$ for data without noise.
...
\item The JUNA project draft defining the coupled coefficients for the partial-FFT views, combiner, and LDPC parity objective.
\end{enumerate}
