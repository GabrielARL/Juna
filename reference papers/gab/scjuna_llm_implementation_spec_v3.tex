...
\pagestyle{fancy}
\fancyhf{}
\lhead{SC-JUNA v3 implementation spec}
...
\title{SC-JUNA v3: Implementation-First Julia Specification\\
\large A deterministic receiver spec for LLM-assisted coding}
\author{Self-calibrating JUNA receiver for Doppler-distorted OFDM}
\date{\today}
...
which is the document to implement from. Three differences with the deployed
receiver were the reason: this specification steps the bit latents directly
(\code{state.z .-= cfg.z_stepsize .* grad_z}) while the deployed receiver uses
Adam, as the manual gradient plan requires; the LLR erasure stage described here
is not present in the deployed receiver; and the shared constants here
(\code{RHO_MAX}, \code{LAMBDA_P_MAX}, ramp constants) differ from the manual
gradient plan and are applied to a loss that is normalized differently, so the
two sets of numbers are not comparable. Marked on 31 July 2026.
\end{minipage}}
\end{center}

\begin{center}
\fbox{\begin{minipage}{0.93\linewidth}
\textbf{Purpose.} This is a code-generation specification, not a research exposition. It deliberately removes ambiguous derivation-first prose and replaces it with exact defaults, data shapes, edge rules, file schemas, concrete receiver APIs, deterministic candidate selection, manual gradients, and golden-vector tests. Version 1 assumes one aligned CP-included OFDM block per packet. Preamble acquisition is not implemented here.
\end{minipage}}
\end{center}
...
The earlier implementation spec had the right architecture but still left too much freedom at exactly the places where generated code tends to hallucinate: free constants, diagnostic thresholds, pilot holdout, LDPC file format, interleaver order, partial-FFT windows, known-Doppler validation, and the $z$-gradient. This v3 spec makes the following hard choices.
...
LDPC parser and encoder unspecified & v1 uses a systematic sparse-coordinate TOML schema with $H=[A\ I]$ and encoder $p=A u$ over $\F_2$. \\
...
Known-Doppler benchmark lacked a self-test & A deterministic Doppler undo golden test is included. \\
AD choice ambiguous & Production uses a hand-derived real gradient for $z$. Zygote is only an optional gradient checker. \\
Missing numerical primitive tests & Golden-vector tests are specified for OFDM, partial FFT, channel, LDPC, RPC, known Doppler, and JUNA gradients. \\
\bottomrule
\end{longtable}

\section{Version-1 scope and hard constraints}

\subsection{In scope}

\begin{enumerate}
\item Julia simulation of one CP-OFDM packet through a time-varying underwater acoustic channel.
\item Full-FFT, RPC, known-Doppler, fixed-JUNA, and self-calibrating JUNA receivers.
...
\subsection{Non-negotiable implementation choices}
...
\code{GAMMA_C} & $10^{-2}$ & Branch-model ridge. \\
...
\subsection{No hidden adaptive gates}
...
\clearpage
\section{Indexing and exact array shapes}
\label{sec:indexing}
...
\end{cases}
\end{equation}
The physical subcarrier frequency used only in passband-aware Doppler formulas is
\begin{equation}
...
If $|\mathcal P|\ge 8\cdot \code{MIN_HOLDOUT_PILOTS}$, validation pilots are every eighth pilot in pilot-list order. Otherwise, no pilot is held out; validation residual is reported as \code{NaN} and scoring ignores it.
...
\subsection{Systematic sparse-coordinate TOML schema}
...
\subsection{Channel golden tests}
...
\item \textbf{Fractional delay test.} Input ramp $x[n]=n$ and delay $d=2.5$. Verify linear interpolation against hand-computed values.
\item \textbf{No-Doppler OFDM diagonal test.} Static CP-covered channel must produce negligible off-diagonal empirical $H_{kq}$ in a diagnostic basis-probe test.
\end{enumerate}

\clearpage
\section{Known-Doppler compensation benchmark}

This benchmark is a simulation-only oracle. SC-JUNA never receives \code{ChannelTruth}.
...
\end{equation}
For baseband-only v1 tests, set \code{use_carrier_phase=false}.
...
\subsection{Mandatory known-Doppler self-test}

Use a no-channel, no-noise packet $x$. Apply a known synthetic global time scale $a=5\times10^{-5}$ with the same interpolation convention. Then run \code{compensate_known_doppler}. Verify
\begin{equation}
\frac{\norm{\tilde x-x}_2}{\norm{x}_2}<10^{-3}
\end{equation}
...
\clearpage
\section{Receiver specifications}
...
\subsection{Convert symbol gradient to bit-latent gradient}
...
\end{cases}
\end{equation}
For v1, smoothness $\gamma_S$ is applied by one post-update smoothing pass:
\begin{equation}
\mathbf w_k\leftarrow \frac{\mathbf w_k+\alpha_s(\mathbf w_{k-1}+\mathbf w_{k+1})}{1+2\alpha_s},
\qquad
\alpha_s=\min(0.25,\gamma_S).
\end{equation}
At edges, use the available neighbor only. This avoids a block-tridiagonal implementation in v1 while making $\gamma_S$ deterministic. A later version may replace this with the exact block-tridiagonal solve.
...
\subsection{Deterministic base $M_0$ and $K_0$ rules}
...
\clearpage
\section{Concrete SC-JUNA receiver pseudocode}
...
Use the mandatory test in the known-Doppler section. This catches sign, unit, and off-by-one mistakes.

\subsection{RPC synthetic test}

Construct $Y_{k,m}=a_m X_k$ for all carriers with known branch vector $a$. Fit RPC on pilots. Verify $w^Ha\approx1$ and pilot residual below $10^{-10}$ in noise-free data.
...
\item \code{SCJUNAReceiver}: self-calibrating JUNA.
\end{enumerate}
...
\begin{enumerate}
\item All golden-vector tests pass.
...
\clearpage
\appendix
\section{Minimal background equations}

This appendix is intentionally short. The implementation should rely on the main-body function definitions, not on re-deriving these equations.
...
Y_k=\sum_qH_{kq}X_q+\nu_k.
\end{equation}
If the channel is time-invariant over the OFDM symbol, $H_{kq}$ is diagonal. If Doppler or path delays vary within the symbol, off-diagonal ICI appears. Partial FFT preserves time-local observations
\begin{equation}
Y_{k,m}=\frac{1}{\sqrt N}\sum_{n=0}^{N-1}g_m[n]y[n]e^{-j2\pi(k-1)n/N},
\end{equation}
...
\item The JUNA project draft defining the coupled branch-model, combiner, and LDPC parity objective.
\end{enumerate}
